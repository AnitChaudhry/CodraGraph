...
- **OpenAI-compatible**: packages/harness's `OpenAIInferenceProvider` works as-is by setting `baseURL` to vLLM's endpoint
...
The `infra/benchmarking/docker/docker-compose.yaml` uses vLLM by default. To run standalone:
...
vLLM exposes an OpenAI-compatible `/v1/chat/completions` endpoint. The `OpenAIInferenceProvider` in `packages/harness/src/inference/openai.ts` works directly:

```ts
import { OpenAIInferenceProvider } from "packages/harness/inference";
