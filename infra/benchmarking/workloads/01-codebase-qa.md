...
`packages/harness/test/fixtures/qa-test-set.json` — 105 hand-labeled tasks. Maintained as the workload's `v1`.
...
```json
{
  ...
  "expectedAnswer": "packages/core/src/core/ingestion/pipeline-phases/parse.ts",
...
This split is enforced by the bench script: when running a `packages/harness-tuned` or `codragraph-swarm-tuned` treatment, the harness search uses tasks 1–80; the final scores published are on tasks 81–105.
...
4. Document changes in `infra/benchmarking/workloads/CHANGELOG.md`
