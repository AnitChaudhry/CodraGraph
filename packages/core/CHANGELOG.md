...
**Major release.** Index schema version is now `2`. Existing 1.7.x / 1.6.x indexes go through a one-time full re-analyze on the first run with this CLI — that's the breaking change. When `--compress` is left unset, the on-disk format is byte-identical to 1.7.x, so the upgrade is "run `codragraph analyze` once" with no further migration. `@codragraph/graphstore` jumps `0.2.0 → 1.0.0` to mark the same stable-format commitment for downstream consumers.

### Added

- **`codragraph analyze --compress brotli|zstd|none`** — opt-in per-row body compression for indexes that have outgrown the default size. Brotli runs at quality 6 on every Node ≥ 18, zstd at level 3 on Node ≥ 22.15. Compression params are pinned so the same input always produces byte-identical output — a load-bearing property for content-addressed snapshots. CLI validates the value before any work and surfaces a clear "use brotli on older Node" hint when zstd isn't available.
- **`codragraph profile-heap [path]`** — new subcommand that wraps `analyze` with v8 heap-snapshot instrumentation and a crash-safe `process.memoryUsage()` JSONL timeline. After the run, prints a per-phase RSS / `heapUsed` summary table and points at the raw `.heapsnapshot` files for Chrome DevTools (Memory → Load) deep dives. Discoverable via `codragraph --help`.
- **`@codragraph/graphstore` 1.0.0:** new `encodeContent`, `decodeContent`, and `isEncodingSupported` exports for `'brotli'` and `'zstd'`. `SUPPORTED_DECODE_ENCODINGS` is computed at module load — always includes `'none'` and `'brotli'`, adds `'zstd'` only when the runtime exposes native zstd. Older Node receiving a zstd-tagged row hits the forward-compat error path with a clear upgrade message rather than producing wrong content.
- **Schema:** every node table with `content` (File, Function, Class, Interface, Method, CodeElement, Section, plus all 20 multi-language tables) gained a `contentEncoding STRING DEFAULT 'none'` column. Default keeps existing reads working unchanged. Tables without content (Folder, Community, Process, Route, Tool) are intentionally untouched.

### Changed

- **BM25 / FTS is now compression-aware.** When a repo was analysed with `--compress brotli|zstd`, the FTS index drops `content` from its property list and falls back to symbol-name matches — search is narrower but never wrong (instead of silently tokenising base64-of-encoded-bytes). The legacy / unset-compress path keeps the full `['name', 'content']` index unchanged.
- **`RepoMeta.compress` is persisted** by every `analyze` run (`'none'` when the flag is unset). Used by the FTS layer above; CLI status / future tooling can detect compressed mode without sampling rows.
- **Read-side decode is centralised** in a new `core/cgdb/content-read.ts` helper (`decodeContentField`). Every external-consumer boundary that returns `content` runs through it: MCP local-backend (single + multi-symbol fetch), HTTP API node stream, the embeddings pipeline. CLI tools (`query`, `context`, `impact`, `wiki`) inherit the shims via the MCP path.
- **`runFullAnalysis`** now writes a `profile-summary.jsonl` timeline at every phase boundary when heap-profile mode is on, alongside the v8 snapshots. The cheap RSS curve is captured first and appended before the snapshot write attempts — if the snapshot fails (out of disk, permissions), the timeline still survives.

### Fixed

- **Windows `0xC0000005` (ACCESS_VIOLATION) crash on `codragraph analyze`** — bumped `@ladybugdb/core` `0.15.4 → 0.16.0`, which fixes a native-binding teardown crash that hit every analyze subprocess on Windows + Node 22.14. Included the production path: `npm install -g @codragraph/cli && codragraph analyze .` from PowerShell or cmd. Reproducible standalone outside any test harness on 0.15.4; deterministic exit-0 across all launch paths on 0.16.0. The 11 `cli-e2e` tests previously skipped on Windows now pass cleanly (24/24).
- **Hash stability across encodings.** A row with no encoding field, an explicit `contentEncoding: 'none'`, and a `brotli`/`zstd`-encoded row of the same logical content all canonicalise to identical CAS ids — the load-bearing invariant that detect-changes / blame / diff depend on. Tests exercise the round-trip end-to-end through `normalizeRowForHash`.
- **`closeCgdb()` close-order discipline.** Every `QueryResult` and `PreparedStatement` returned by the connection is tracked in a `liveCgdbHandles` Set via a `trackConnection()` wrapper, and drained before `conn.close()` runs. Per-call-site `closeQueryResult()` calls in the hot paths (executeQuery, executePrepared, executeWithReusedStatement, getCgdbStats, queryFTS) keep the Set small during a session.
- **Vitest `globalSetup` scoped to the `cgdb-db` project only.** Prior config opened a LadybugDB native binding before any test ran, which segfaulted the worker fork on Windows + Git Bash and hung the whole `vitest run` invocation. The unit suite (~6 000 tests) now runs locally on Windows.
- **Workspace-hoisted postinstall scripts.** `patch-tree-sitter-swift.cjs` and `build-tree-sitter-proto.cjs` now resolve from both `packages/core/node_modules/<dep>` and the workspace root. Without this fix, a workspace install left the Swift binding unbuilt and `require('tree-sitter-swift')` threw "No native build was found" on Node 22.14. Both bindings now load cleanly after a fresh install.

## [1.7.0] - 2026-04-30

### Added

- **`npx @codragraph/cli analyze` is a true zero-install entry.** First-run detection (missing `~/.codragraph/registry.json`) auto-runs editor setup before indexing, so users no longer need a separate `codragraph setup` step. Opt out with `--no-setup` for CI / headless contexts. The README's TL;DR is now one command.
- **Configurable large-file skip threshold** — the walker's 512 KB default is overridable via `CODRAGRAPH_MAX_FILE_SIZE` (KB) or `codragraph analyze --max-file-size <kb>`. Values are clamped to the 32 MB tree-sitter ceiling, invalid inputs fall back to the default with a one-time warning, and the CLI banner reports the effective post-clamp threshold when an override is active (#991, #1044).
- **`--skill-targets <list>` for `codragraph analyze --skills`.** Per-community SKILL.md files now emit to multiple editor targets in one pass — `claude`, `cursor`, `opencode`, `codex` — under each editor's project skill convention (`.claude/skills/generated/`, `.cursor/skills/generated/`, `.opencode/skill/generated/`, `.codex/skills/generated/`). Default stays `claude` (matches pre-flag behavior).
- **`codragraph diff --json`** for both plain and `--semantic` modes. Stable machine-readable shape for CI / GitHub Action / IDE plugin consumers; the human-readable text format is unchanged when `--json` is absent.
- **Token-savings reporter on every CLI tool result.** `codragraph query`, `context`, `impact`, and the `analyze` summary now end with `@codragraph/compress: ~N tokens of structured context (vs ~M raw — P% smaller).` The estimate uses the same chars/4 heuristic as `@codragraph/compress`'s `estimateTokens`, computed against the JSON response and the on-disk file size of every file referenced in the response.
- **Per-skill token counts during `--skills` generation.** Each generated SKILL.md prints its token estimate so agents can see the distillation ratio per community.
- **Background auto-reindex on `git commit` / `merge` / `rebase` / `pull`.** Opt-in via `CODRAGRAPH_AUTO_REINDEX=1` *or* `~/.codragraph/config.json#autoReindex` (the config-file fallback is Windows-GUI-launcher safe; env vars don't always propagate to hook child processes). Default behavior remains the notification-only path. The Claude Code PostToolUse hook spawns a detached `analyze --no-setup`; a `.codragraph/.reindex.coalesce` file gates concurrent runs (single in-flight reindex per repo), and the spawned analyze removes it on exit so subsequent commits trigger reindex immediately. A 10-min mtime crash-safety TTL handles ungraceful exits.
- **PR review GitHub Action** at `.github/actions/codragraph-pr-review/`. Posts a sticky structural-diff comment on every PR using `codragraph diff --semantic --json`. Two modes: `mode: deterministic` (default, free) emits a Markdown summary with risk heuristic; `mode: review` (opt-in) additionally calls Anthropic with the diff JSON for a richer human-readable review on top, falling back to the deterministic comment on any API failure. Sample workflow at `.github/workflows/codragraph-pr-review-example.yml` (gated by `if: false` until consumers have 1.7.0+ on npm).
- **22 new built-in workflow skills** (17 → 32 total, auto-installed by `codragraph setup` to all four editor target dirs):
  - **Dev power-tools (5):** `codragraph-test-coverage`, `codragraph-dead-code`, `codragraph-api-surface`, `codragraph-onboarding`, `codragraph-migration-tracking`.
  - **Data engineering (3):** `codragraph-data-lineage`, `codragraph-sql-tracing`, `codragraph-notebook-context`.
  - **Multi-project / vibecoding (2):** `codragraph-project-switcher`, `codragraph-cross-repo-impact`.
  - **Git workflow (6):** `codragraph-git-rebase-vs-merge`, `codragraph-git-force-push`, `codragraph-git-recovery`, `codragraph-git-bisect`, `codragraph-git-worktree`, `codragraph-git-history-rewrite`.
  - **GitHub CLI workflow (4):** `codragraph-gh-pr-workflow`, `codragraph-gh-issue-workflow`, `codragraph-gh-actions-debug`, `codragraph-gh-release-workflow`.
  - **Domain — security / perf / ops (5):** `codragraph-security-audit`, `codragraph-perf-hotspots`, `codragraph-observability-coverage`, `codragraph-config-audit`, `codragraph-supply-chain-audit`.
...
- `setupCommand` is now a thin wrapper around `runSetup({skipNextSteps?, compactHeader?})`. Programmatic callers (e.g. analyze's first-run path) use `runSetup({skipNextSteps: true, compactHeader: true})` to invoke setup inline without the trailing "Next steps" guidance. Public `setupCommand()` signature is unchanged.

### Snapshot hash stability (foundation for the 2.0 compression work)

- `@codragraph/graphstore` (bumped 0.1.2 → 0.2.0): new `normalizeRowForHash` and `decodeContent` exports plus the `ContentEncoding` type. The serializer now routes every row through `normalizeRowForHash` before hashing — so a row with no encoding field, an explicit `contentEncoding: 'none'`, and (eventually) a compressed row all produce identical CAS ids. The cornerstone correctness invariant the upcoming compression work depends on.
- Identity-op for 1.7.x users: no row carries `contentEncoding` yet, so existing 1.6.x indexes are byte-identical to 1.7.x snapshots of the same source.
- Tests: unit + serializer-integration coverage of the hash-stability invariant.

### Heap-profile instrumentation

- New `CODRAGRAPH_HEAP_PROFILE=1` env var. When set, `runFullAnalysis` writes a `v8.writeHeapSnapshot()` at every distinct phase boundary into `<repo>/.codragraph/heap-profiles/<unix-ms>-<phase>.heapsnapshot`. Off by default — each snapshot pauses analyze for 2–5 s and writes 100 MB–2 GB of disk. Open the snapshots in Chrome DevTools → Memory → Load to find which constructors dominate retained set at each phase boundary.

### Upgrade notes

- Most users: `npm i -g @codragraph/cli@1.7.0`. No breaking changes; everything else is additive.
- Consumers of the new GitHub Action: pin to a tagged release (e.g. `@v1.7.0`) once published, or `@main` until then.
- Existing `~/.codragraph/registry.json` users won't see the first-run setup re-run — analyze detects the registry and skips. To force a re-setup, run `codragraph setup` directly.
...
- **Global-install upgrade `ENOTEMPTY`** — devendored `tree-sitter-proto` install lifecycle + preinstall cleanup so `npm i -g codragraph@latest` succeeds on top of an older install (#843, #846)
...
- apps/web dependency bumps: `mermaid` 11.12.2 → 11.14.0 (#860), `tailwindcss` (#861), `jsdom` 29.0.0 → 29.0.2 (#863), `wait-on` 8.0.5 → 9.0.5 (#859), `@vitest/coverage-v8` (#864)
...
- **Runtime dependency on `file:../shared`** removed from the published package (#803)
...
- **Service group infrastructure** — service boundary detection, contract extractors, sync pipeline, CLI/MCP tools, monorepo fixture; bridge.cgdb storage and contract matching expansion (#795)
...
- **`CODRAGRAPH_HOME` env var** to customize global directory (#746)
...
- **`.codragraphignore` negation patterns** now respected (#654)
- **MCP setup** prefers global codragraph binary over npx (#653)
...
### Fixed
- **`packages/shared` module not found** — `packages/shared` was a `file:` workspace dependency never published to npm, causing `ERR_MODULE_NOT_FOUND` when installing `codragraph` globally. The build now bundles shared code into `dist/_shared/` and rewrites imports to relative paths (#613)
...
### Fixed
- Incomplete fix for `packages/shared` bundling — `prepare` script overwrote rewritten imports during publish
...
- **Unified web & CLI ingestion pipeline** — complete architectural migration of the web app from a self-contained WASM browser app to a thin client backed by the CLI server; new `packages/shared` package for cross-package type unification (#536)
...
- **`--skip-agents-md` CLI flag** — opt out of overwriting CodraGraph-managed sections in AGENTS.md and CLAUDE.md during `codragraph analyze` (#517)
...
### Docs
- Added `packages/shared` build step to web UI quick start instructions (#585)
- Added enterprise offering section to README (#579)

## [1.4.10] - 2026-03-27

### Fixed
- **MCP server install via npx** — resolve tree-sitter peer dependency conflicts that broke `npx -y codragraph@latest mcp` (#537, #538)
...
- **CI** — shape-check-regression test moved to cgdb-db project (#518)
...
- `codragraph-stable-ops` added to community integrations
...
- `.gitignore` and `.codragraphignore` support during file discovery (#231)
