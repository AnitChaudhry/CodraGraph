...
describe('registry persists remoteUrl', () => {
  ...
  beforeEach(async () => {
    tmpHome = await createTempDir('codragraph-sibling-home-');
    tmpRepo = await createTempDir('codragraph-sibling-repo-');
    savedHome = process.env.CODRAGRAPH_HOME;
    process.env.CODRAGRAPH_HOME = tmpHome.dbPath;
  });

  afterEach(async () => {
    if (savedHome === undefined) delete process.env.CODRAGRAPH_HOME;
    else process.env.CODRAGRAPH_HOME = savedHome;
...
describe('findSiblingClones', () => {
  ...
  beforeEach(async () => {
    tmpHome = await createTempDir('codragraph-sibling-find-home-');
    savedHome = process.env.CODRAGRAPH_HOME;
    process.env.CODRAGRAPH_HOME = tmpHome.dbPath;
  });
...
describe('checkCwdMatch', () => {
  ...
  beforeEach(async () => {
    tmpHome = await createTempDir('codragraph-cwd-match-home-');
    savedHome = process.env.CODRAGRAPH_HOME;
    process.env.CODRAGRAPH_HOME = tmpHome.dbPath;
  });
