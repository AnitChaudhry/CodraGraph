...
// local-backend.ts imports from core/cgdb/pool-adapter.js; the mcp/core/cgdb-adapter.js
...
const { cgdbMocks } = vi.hoisted(() => ({
  cgdbMocks: {
    initCgdb: vi.fn().mockResolvedValue(undefined),
    executeQuery: vi.fn().mockResolvedValue([]),
    executeParameterized: vi.fn().mockResolvedValue([]),
    closeCgdb: vi.fn().mockResolvedValue(undefined),
    isCgdbReady: vi.fn().mockReturnValue(true),
  },
}));

vi.mock('../../src/core/cgdb/pool-adapter.js', async (importOriginal) => {
  const actual = await importOriginal();
  return { ...actual, ...cgdbMocks };
});

// Re-export shim must resolve to the same mocks
vi.mock('../../src/mcp/core/cgdb-adapter.js', async (importOriginal) => {
  const actual = await importOriginal();
  return { ...actual, ...cgdbMocks };
});
...
vi.mock('../../src/core/search/bm25-index.js', () => ({
  searchFTSFromCgdb: vi.fn().mockResolvedValue([]),
}));
...
const MOCK_REPO_ENTRY = {
  ...
  storagePath: '/tmp/.codragraph/test-project',
...
function setupMultipleRepos() {
  (listRegisteredRepos as any).mockResolvedValue([
    MOCK_REPO_ENTRY,
    {
      ...
      storagePath: '/tmp/.codragraph/other-project',
    },
  ]);
}
...
describe('LocalBackend.disconnect', () => {
  ...
  it('calls closeCgdb on disconnect', async () => {
    setupSingleRepo();
    await backend.init();
    await backend.disconnect();
    expect(closeCgdb).toHaveBeenCalled();
  });
});
...
describe('ensureInitialized', () => {
  ...
  it('calls initCgdb on first tool call', async () => {
    (executeParameterized as any).mockResolvedValue([]);
    await backend.callTool('query', { query: 'test' });
    expect(initCgdb).toHaveBeenCalled();
  });

  it('retries initCgdb if connection was evicted', async () => {
    (executeParameterized as any).mockResolvedValue([]);
    // First call initializes
    await backend.callTool('query', { query: 'test' });
    expect(initCgdb).toHaveBeenCalledTimes(1);

    // Simulate idle eviction
    (isCgdbReady as any).mockReturnValueOnce(false);
    await backend.callTool('query', { query: 'test' });
    expect(initCgdb).toHaveBeenCalledTimes(2);
  });

  it('handles initCgdb failure gracefully', async () => {
    (initCgdb as any).mockRejectedValueOnce(new Error('DB locked'));
...
describe('cypher tool LadybugDB not ready', () => {
  ...
  it('returns error when LadybugDB is not ready', async () => {
    (isCgdbReady as any).mockReturnValue(false);
    // initCgdb will succeed but isCgdbReady returns false after ensureInitialized
    // Actually ensureInitialized checks isCgdbReady and re-inits — let's make that pass
    // then the cypher method checks isCgdbReady again
    (isCgdbReady as any)
...
describe('cypher result formatting', () => {
  ...
  beforeEach(async () => {
    ...
    (initCgdb as any).mockResolvedValue(undefined);
    (isCgdbReady as any).mockReturnValue(true);
    (closeCgdb as any).mockResolvedValue(undefined);
    (executeParameterized as any).mockResolvedValue([]);
