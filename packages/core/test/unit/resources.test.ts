...
describe('getResourceDefinitions', () => {
  ...
  it('includes repos resource', () => {
    const defs = getResourceDefinitions();
    const repos = defs.find((d) => d.uri === 'codragraph://repos');
  ...
  it('includes setup resource', () => {
    const defs = getResourceDefinitions();
    const setup = defs.find((d) => d.uri === 'codragraph://setup');
...
describe('getResourceTemplates', () => {
  ...
  it('includes per-repo + graphstore + recipes + group templates', () => {
    ...
    expect(uris).toContain('codragraph://group/{name}/status');
  });
...
describe('parseResourceUri', () => {
  it('parses group contracts without query', () => {
    const p = parseResourceUri('codragraph://group/acme/contracts');
    expect(p).toEqual({
      kind: 'group',
      groupName: 'acme',
  ...
  it('parses nested group name and contracts query params', () => {
    const p = parseResourceUri(
      'codragraph://group/acme/billing/contracts?type=http&repo=app%2Fapi&unmatchedOnly=true',
    );
  ...
  it('coerces unmatchedOnly false from string', () => {
    const p = parseResourceUri('codragraph://group/g1/contracts?unmatchedOnly=false');
    expect(p.kind).toBe('group');
    if (p.kind !== 'group' || p.resourceType !== 'contracts') throw new Error('unexpected');
  ...
  it('parses group status', () => {
    const p = parseResourceUri('codragraph://group/my/product/status');
    expect(p).toEqual({
      kind: 'group',
  ...
  it('round-trips repo URI like legacy regex', () => {
    const p = parseResourceUri('codragraph://repo/my%20project/schema');
    expect(p).toEqual({
      kind: 'repo',
      repoName: 'my project',
      resourceType: 'schema',
...
describe('readResource', () => {
  it('routes codragraph://repos to listRepos', async () => {
    ...
    const result = await readResource('codragraph://repos', backend);
  ...
  it('routes codragraph://setup to setup resource', async () => {
    const backend = createMockBackend({
      repos: [
        {
          name: 'proj',
  ...
  it('routes group contracts resource through backend', async () => {
    const backend = createMockBackend();
    const uri = 'codragraph://group/g1/contracts?type=http&unmatchedOnly=true';
  ...
  it('routes group status resource through backend', async () => {
    const backend = createMockBackend();
    await readResource('codragraph://group/acme/status', backend);
    expect(backend.readGroupStatusResource).toHaveBeenCalledWith('acme');
  });

  it('routes codragraph://repo/{name}/context correctly', async () => {
    const backend = createMockBackend({
      context: {
    ...
    const result = await readResource('codragraph://repo/test-project/context', backend);
  ...
  it('routes codragraph://repo/{name}/schema to static schema', async () => {
    const backend = createMockBackend();
    const result = await readResource('codragraph://repo/any/schema', backend);
    expect(result).toContain('CodraGraph Graph Schema');
    expect(result).toContain('CALLS');
    expect(result).toContain('IMPORTS');
  });

  it('routes codragraph://repo/{name}/clusters correctly', async () => {
    const backend = createMockBackend({
      clusters: {
  ...
  it('routes codragraph://repo/{name}/processes correctly', async () => {
    const backend = createMockBackend({
      processes: {
        processes: [{ heuristicLabel: 'LoginFlow', processType: 'intra_community', stepCount: 3 }],
      },
    });
    const result = await readResource('codragraph://repo/test/processes', backend);
  ...
  it('routes codragraph://repo/{name}/cluster/{clusterName} correctly', async () => {
    const backend = createMockBackend({
      clusterDetail: {
    ...
    const result = await readResource('codragraph://repo/test/cluster/Auth', backend);
  ...
  it('handles cluster detail error', async () => {
    ...
    const result = await readResource('codragraph://repo/test/cluster/Missing', backend);
    expect(result).toContain('Cluster not found');
  });

  it('routes codragraph://repo/{name}/process/{processName} correctly', async () => {
    const backend = createMockBackend({
      processDetail: {
    ...
    const result = await readResource('codragraph://repo/test/process/LoginFlow', backend);
  ...
  it('handles process detail error', async () => {
    ...
    const result = await readResource('codragraph://repo/test/process/Missing', backend);
    expect(result).toContain('Process not found');
  });

  it('throws for unknown resource URI', async () => {
    const backend = createMockBackend();
    await expect(readResource('codragraph://unknown', backend)).rejects.toThrow(
  ...
  it('throws for unknown repo-scoped resource type', async () => {
    const backend = createMockBackend();
    await expect(readResource('codragraph://repo/test/nonexistent', backend)).rejects.toThrow(
      'Unknown resource',
    );
  });
  ...
  it('decodes URI-encoded repo names', async () => {
    const backend = createMockBackend();
    await readResource('codragraph://repo/my%20project/schema', backend);
  ...
  it('decodes URI-encoded cluster names', async () => {
    ...
    await readResource('codragraph://repo/test/cluster/Auth%20Module', backend);
