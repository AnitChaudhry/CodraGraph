...
describe('getStoragePath', () => {
  it('appends .codragraph to resolved repo path', () => {
    const result = getStoragePath('/home/user/project');
    expect(result).toContain('.codragraph');
    expect(path.basename(result)).toBe('.codragraph');
  });
...
describe('getStoragePaths', () => {
  it('returns storagePath, cgdbPath, metaPath', () => {
    const paths = getStoragePaths('/home/user/project');
    expect(paths.storagePath).toContain('.codragraph');
    expect(paths.cgdbPath).toContain('cgdb');
    expect(paths.metaPath).toContain('meta.json');
  });

  it('all paths are under storagePath', () => {
    const paths = getStoragePaths('/home/user/project');
    expect(paths.cgdbPath.startsWith(paths.storagePath)).toBe(true);
...
describe('readRegistry', () => {
  it('returns empty array when registry does not exist', async () => {
    // readRegistry reads from ~/.codragraph/registry.json
...
describe('saveCLIConfig / loadCLIConfig', () => {
  ...
  beforeEach(async () => {
    tmpHandle = await createTempDir('codragraph-config-test-');
...
//
// Each test isolates the global registry by pointing CODRAGRAPH_HOME at a
...
describe('registerRepo name override + collision guard (#829)', () => {
  ...
  let savedCodragraphHome: string | undefined;
  ...
  beforeEach(async () => {
    tmpHome = await createTempDir('codragraph-registry-home-');
    tmpRepoA = await createTempDir('codragraph-repo-a-');
    tmpRepoB = await createTempDir('codragraph-repo-b-');
    savedCodragraphHome = process.env.CODRAGRAPH_HOME;
    process.env.CODRAGRAPH_HOME = tmpHome.dbPath;
  });

  afterEach(async () => {
    if (savedCodragraphHome === undefined) delete process.env.CODRAGRAPH_HOME;
    else process.env.CODRAGRAPH_HOME = savedCodragraphHome;
  ...
  it('basename collisions without an explicit --name still register silently (backward-compat)', async () => {
    ...
    const parentA = await createTempDir('codragraph-collide-parent-a-');
    const parentB = await createTempDir('codragraph-collide-parent-b-');
...
describe('getInferredRepoName + registerRepo (#979 — git remote inference)', () => {
  ...
  beforeEach(async () => {
    tmpHome = await createTempDir('codragraph-registry-home-979-');
    savedCodragraphHome = process.env.CODRAGRAPH_HOME;
    process.env.CODRAGRAPH_HOME = tmpHome.dbPath;
  });
  ...
  it('getInferredRepoName returns null when there is no .git directory', async () => {
    const tmp = await createTempDir('codragraph-no-git-');
    try {
  ...
  it('getInferredRepoName returns null when origin is unset', async () => {
    const tmp = await createTempDir('codragraph-no-origin-');
    try {
  ...
  it('getInferredRepoName returns the remote repo name when origin is set', async () => {
    const tmp = await createTempDir('codragraph-with-origin-');
    try {
  ...
  it('registerRepo derives name from git remote when basename is generic (Gas-Town repro)', async () => {
    ...
    const root = await createTempDir('codragraph-gastown-');
    try {
  ...
  it('two analyze calls of differently-remoted "rig" leaves no longer collide', async () => {
    ...
    const root = await createTempDir('codragraph-gastown-2-');
    try {
  ...
  it('explicit --name still wins over remote inference', async () => {
    const tmp = await createTempDir('codragraph-name-wins-');
    try {
  ...
  it('preserved alias still wins over remote inference on re-analyze', async () => {
    const tmp = await createTempDir('codragraph-preserve-alias-');
    try {
      initGitRepo(tmp.dbPath, 'https://github.com/owner/from-remote.git');
  ...
  it('falls back to basename when no .git / no remote is available', async () => {
    const tmp = await createTempDir('codragraph-fallback-basename-');
    try {
      const name = await registerRepo(tmp.dbPath, meta);
...
// ─── resolveRegistryEntry (#664 — codragraph remove <target>) ──────────
//
...
// ~/.codragraph. No CODRAGRAPH_HOME sandboxing needed. This also means the
  ...
  const entries: RegistryEntry[] = [
    {
      name: 'app',
      path: pathA,
      storagePath: `${pathA}${path.sep}.codragraph`,
    ...
    },
    {
      name: 'app',
      path: pathB,
      storagePath: `${pathB}${path.sep}.codragraph`,
      indexedAt: '2026-04-18T00:00:00.000Z',
      lastCommit: 'bbbbbbb',
    },
    {
      name: 'website',
      path: pathW,
      storagePath: `${pathW}${path.sep}.codragraph`,
      indexedAt: '2026-04-18T00:00:00.000Z',
      lastCommit: 'ccccccc',
    },
  ];
  ...
  it('path match wins over name match (never ambiguous)', () => {
    ...
    const weird: RegistryEntry[] = [
      { ...entries[2] }, // 'website' at pathW
      {
        ...
        storagePath: `${prefix}elsewhere${path.sep}odd${path.sep}.codragraph`,
        indexedAt: '2026-04-18T00:00:00.000Z',
        lastCommit: 'ddddddd',
      },
    ];
...
describe('resolveRegistryEntry backward-compat with non-canonical stored paths (#1003)', () => {
  it('matches a stored entry even when the target was passed in canonical form', async () => {
    ...
    const nonCanonical = realDir + path.sep + '.'; // e.g. /work/codragraph/.
    ...
    const entries: RegistryEntry[] = [
      {
        ...
        path: nonCanonical,
        storagePath: path.join(nonCanonical, '.codragraph'),
...
//
// Guard rail against destroying more than the `.codragraph/` subfolder.
// `~/.codragraph/registry.json` is user-writable plain text, so a
...
describe('assertSafeStoragePath (#1003)', () => {
  ...
  it('accepts the canonical <repo>/.codragraph storage path', () => {
    const entry: RegistryEntry = {
      ...base,
      storagePath: path.join(repoPath, '.codragraph'),
    };
  ...
  it('rejects when storagePath is a sibling .codragraph (right basename, wrong parent)', () => {
    const entry: RegistryEntry = {
      ...base,
      storagePath: path.join(`${prefix}different${path.sep}repo`, '.codragraph'),
    };
    expect(() => assertSafeStoragePath(entry)).toThrow(UnsafeStoragePathError);
  });
  ...
  it('UnsafeStoragePathError carries the original entry + expected + actual paths', () => {
    ...
    try {
      ...
      // Expected path is the canonical `<repo>/.codragraph`.
      expect(err.expectedStoragePath).toBe(path.join(path.resolve(repoPath), '.codragraph'));
  ...
  it('Windows: storagePath match is case-insensitive to match register/unregister semantics', () => {
    ...
    const entry: RegistryEntry = {
      ...base,
      storagePath: path.join(repoPath.toUpperCase(), '.CODRAGRAPH'),
    };
