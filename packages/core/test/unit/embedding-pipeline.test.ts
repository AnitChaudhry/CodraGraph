...
import { STALE_HASH_SENTINEL } from '../../src/core/cgdb/schema.js';
...
describe('EMBEDDING_SCHEMA', () => {
  it('includes contentHash STRING column', async () => {
    const { EMBEDDING_SCHEMA } = await import('../../src/core/cgdb/schema.js');
...
describe('EMBEDDING_INDEX_NAME', () => {
  it('is exported from schema.ts', async () => {
    const { EMBEDDING_INDEX_NAME } = await import('../../src/core/cgdb/schema.js');
...
describe('runEmbeddingPipeline incremental filter', () => {
  ...
  const mockEmbedderSetup = () => {
    ...
    // Mock loadVectorExtension (avoids needing the native cgdb module)
    vi.doMock('../../src/core/cgdb/cgdb-adapter.js', () => ({
  ...
  it('maps positional query rows with description/isExported columns correctly', async () => {
    ...
    const executeQuery = vi.fn().mockImplementation(async (cypher: string) => {
      queryCalls.push(cypher);
      // RFC 0001 Phase 2 — every content-bearing RETURN now also projects
      // `contentEncoding` immediately after `content`, so the positional
      // tuple shifted: positions are id, name, label, filePath, content,
      // contentEncoding, startLine, endLine, [isExported], description, …
...
// The function is tested via cgdb-core-adapter integration tests which have the
