...
vi.mock('../../src/storage/git.js', () => ({
  ...
  // older `.codragraph/meta.json` files. The unit tests don't care
...
describe('indexCommand', () => {
  ...
  beforeEach(() => {
    ...
    mockGetStoragePaths.mockImplementation((repoPath: string) => ({
      storagePath: `${repoPath}/.codragraph`,
      cgdbPath: `${repoPath}/.codragraph/cgdb`,
      metaPath: `${repoPath}/.codragraph/meta.json`,
    }));
  ...
  it('fails when .codragraph folder does not exist', async () => {
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    mockAccess.mockRejectedValueOnce(new Error('missing .codragraph'));
    ...
    expect(logSpy).toHaveBeenCalledWith(
      `  No .codragraph/ folder found at: ${resolvedRepo}/.codragraph`,
    );
  });

  it('fails when cgdb database does not exist', async () => {
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    mockAccess.mockResolvedValueOnce(undefined).mockRejectedValueOnce(new Error('missing cgdb'));
    ...
    expect(process.exitCode).toBe(1);
    expect(logSpy).toHaveBeenCalledWith(
      '  .codragraph/ folder exists but contains no LadybugDB index.',
    );
  });
