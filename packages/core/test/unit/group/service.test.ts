...
function makeTmpGroup(): { tmpDir: string; groupDir: string; cleanup: () => void } {
  const tmpDir = path.join(os.tmpdir(), `codragraph-svc-${Date.now()}`);
...
describe('GroupService', () => {
  describe('groupList', () => {
    it('test_groupList_without_name_returns_group_names', async () => {
      const { _groupDir, cleanup, tmpDir } = makeTmpGroup();
      try {
  ...
  describe('groupQuery', () => {
      ...
      try {
        ...
        const port = makePort({
          resolveRepo: vi.fn(async (name?: string) => {
            ...
            return { id: 'fe', name: 'fe', repoPath: '/tmp', storagePath: '/tmp/.codragraph' };
          }),
    ...
    it('test_groupQuery_subgroupExact_skips_descendant_member_paths', async () => {
      const tmpDir = path.join(os.tmpdir(), `codragraph-svc-nest-${Date.now()}`);
...
`,
  ...
  describe('groupContext', () => {
    ...
    it('test_groupContext_subgroupExact_skips_descendant_member_paths', async () => {
      const tmpDir = path.join(os.tmpdir(), `codragraph-ctx-nest-${Date.now()}`);
      const groupDir = path.join(tmpDir, 'groups', 'nest-group');
      fs.mkdirSync(groupDir, { recursive: true });
      fs.writeFileSync(
        path.join(groupDir, 'group.yaml'),
