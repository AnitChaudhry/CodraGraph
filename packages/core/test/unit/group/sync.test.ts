...
describe('syncGroup', () => {
  ...
  it('per-repo extractorOverride receives repo handle and extracts per repo', async () => {
    ...
    const result = await syncGroup(config, {
      ...
      resolveRepoHandle: async (_name, groupPath) => ({
        ...
        storagePath: '/tmp/' + groupPath + '/.codragraph',
      }),
      skipWrite: true,
    });

    // per-repo override goes through the initCgdb path which will fail
  ...
  it('test_syncGroup_closes_only_opened_pools', async () => {
    ...
    const initSpy = vi.spyOn(poolAdapter, 'initCgdb').mockResolvedValue(undefined);
    const closeSpy = vi.spyOn(poolAdapter, 'closeCgdb').mockImplementation(async (id?: string) => {
    ...
    try {
      ...
      // closeCgdb must have been called at least once with specific pool ids
  ...
  it('writes registry to groupDir when skipWrite is false', async () => {
    const tmpDir = path.join(os.tmpdir(), `codragraph-sync-write-${Date.now()}`);
...
describe('stableRepoPoolId', () => {
  it('returns lowercase name when no collision', () => {
    const entry: RegistryEntry = {
      name: 'MyRepo',
      path: '/a/MyRepo',
      storagePath: '/a/MyRepo/.codragraph',
      indexedAt: '',
      lastCommit: '',
    };
    const all = [entry];
