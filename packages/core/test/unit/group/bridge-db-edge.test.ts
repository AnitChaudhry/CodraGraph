...
// See bridge-db.test.ts for the rationale: read-after-write tests on a
// freshly-written bridge.cgdb hit a Windows-only kernel-level lock-release
// lag in LadybugDB's native binding even with the `openBridgeDbReadOnly`
// retry-with-backoff fallback. CI runs them green on Linux/macOS.
...
describe('bridge-db edge cases', () => {
  ...
  it('test_openBridgeDbReadOnly_version_gate_returns_null_for_incompatible', async () => {
    // Create a dummy bridge.cgdb file so the access check passes
    await fsp.writeFile(path.join(tmpDir, 'bridge.cgdb'), 'dummy');
  ...
  itReadAfterWrite('test_openBridgeDbReadOnly_bak_recovery_restores_bridge', async () => {
    ...
    // Move bridge.cgdb → bridge.cgdb.bak (simulating interrupted swap)
    const dbPath = path.join(tmpDir, 'bridge.cgdb');
    const bakPath = path.join(tmpDir, 'bridge.cgdb.bak');
  ...
  itReadAfterWrite('test_writeBridge_crossLink_with_missing_to_node_silently_skipped', async () => {
