...
describe('resolveLLMConfig', () => {
  ...
  beforeEach(async () => {
    ...
    const configDir = path.join(tmpDir, '.codragraph');
...
describe('--verbose flag', () => {
  const originalEnv = process.env.CODRAGRAPH_VERBOSE;

  afterEach(() => {
    if (originalEnv === undefined) {
      delete process.env.CODRAGRAPH_VERBOSE;
    } else {
      process.env.CODRAGRAPH_VERBOSE = originalEnv;
    }
  });

  it('verboseLog writes to console when CODRAGRAPH_VERBOSE=1', async () => {
    ...
    expect(process.env.CODRAGRAPH_VERBOSE).toBe('1');

    consoleSpy.mockRestore();
  });

  it('verbose is off when CODRAGRAPH_VERBOSE is not set', () => {
    delete process.env.CODRAGRAPH_VERBOSE;
    expect(process.env.CODRAGRAPH_VERBOSE).toBeUndefined();
  });
});
...
describe('WikiGenerator invokeLLM routing', () => {
  ...
  it('routes to callCursorLLM when provider is cursor', async () => {
    ...
    const generator = new WikiGenerator(repoPath, storagePath, path.join(storagePath, 'cgdb'), {
      apiKey: '',
      baseUrl: '',
      model: 'test',
      maxTokens: 1000,
