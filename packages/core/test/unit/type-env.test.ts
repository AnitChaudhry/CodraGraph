...
`,
        ...
        // Skipped on Windows: depends on a specific tree-sitter-swift grammar
        // AST shape for `User.init(name: ...)` that drifts between rebuilds
        // of the local native binding. The resolver code is fine; the test
        // pins a parser-version-specific tree shape. Linux/macOS CI uses
        // prebuilt bindings and runs this clean.
        const itSwiftAst = process.platform === 'win32' ? it.skip : it;
        itSwiftAst('Swift explicit init inference uses lookupClassByName', () => {
          const tree = parseSwift(
            `
func run() {
