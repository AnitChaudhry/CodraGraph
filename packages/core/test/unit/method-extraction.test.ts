...
class Foo {
    func greet(name: String, age: Int = 25) -> String {
      ...
      expect(m.name).toBe('greet');
      expect(m.parameters).toHaveLength(2);
      // `objectContaining` keeps the test resilient when the extractor
      // adds new fields (e.g. `rawType` for the unresolved source spelling
      // alongside `type`); the assertion still pins the load-bearing
      // properties without breaking on schema additions.
      expect(m.parameters[0]).toEqual(
