...
import { searchFTSFromCgdb, type BM25SearchResult } from '../../src/core/search/bm25-index.js';
...
// `searchFTSFromCgdb`. We mock it so we can drive the executor without
// spinning up a real LadybugDB pool.
const mockExecuteQuery = vi.fn();
vi.mock('../../src/core/cgdb/pool-adapter.js', () => ({
  executeQuery: (repoId: string, cypher: string) => mockExecuteQuery(repoId, cypher),
}));

describe('BM25 search', () => {
  describe('searchFTSFromCgdb', () => {
    it('returns empty array when LadybugDB is not initialized', async () => {
      ...
      const results = await searchFTSFromCgdb('test query');
      expect(Array.isArray(results)).toBe(true);
      expect(results).toHaveLength(0);
    });

    it('handles empty query', async () => {
      const results = await searchFTSFromCgdb('');
      expect(Array.isArray(results)).toBe(true);
    });

    it('accepts custom limit parameter', async () => {
      const results = await searchFTSFromCgdb('test', 5);
      expect(Array.isArray(results)).toBe(true);
    });
  });
  ...
  describe('score aggregation', () => {
    ...
    it('sums only top-3 scoring nodes per file when more than 3 match', async () => {
      const { queryFTS } = await import('../../src/core/cgdb/cgdb-adapter.js');
      ...
      const results = await searchFTSFromCgdb('queryset');
    ...
    it('propagates nodeIds for files with fewer than 3 matching nodes', async () => {
      ...
      const results = await searchFTSFromCgdb('model');
    ...
    it('filters out empty nodeIds', async () => {
      ...
      const results = await searchFTSFromCgdb('util');
    ...
    it('merges hits across multiple index tables for the same file', async () => {
      ...
      const results = await searchFTSFromCgdb('auth');
    ...
    it('ranks files by aggregated score descending', async () => {
      ...
      const results = await searchFTSFromCgdb('fn');
  ...
  describe('MCP pool path', () => {
    const REPO = 'test-repo-mcp-pool';

    beforeEach(() => {
      mockExecuteQuery.mockReset();
    });

    it('does not try CREATE_FTS_INDEX from the read-only pool and falls back to bounded scan', async () => {
      mockExecuteQuery.mockImplementation(async (_repo: string, cypher: string) => {
        if (cypher.includes('MATCH (node:Function)')) {
          return [
            {
              node: {
                id: 'func:handler',
      ...
      const results = await searchFTSFromCgdb('handler', 5, REPO);
