...
// `searchFTSFromCgdb`. We mock it so we can drive the executor and the
...
describe('BM25 search', () => {
  ...
  describe('ensureFTS cache (MCP pool path)', () => {
    ...
    it('does NOT cache a transient CREATE_FTS_INDEX failure — second call retries', async () => {
      ...
      const r1 = await searchFTSFromCgdb('anything', 5, REPO);
      ...
      await searchFTSFromCgdb('anything', 5, REPO);
    ...
    it('a pool-close listener fired by the pool adapter invalidates this repo only', async () => {
      ...
      // Prime both repos.
      await searchFTSFromCgdb('anything', 5, REPO);
      await searchFTSFromCgdb('anything', 5, OTHER);
