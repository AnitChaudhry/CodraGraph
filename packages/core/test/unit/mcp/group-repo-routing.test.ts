...
vi.mock('../../../src/core/cgdb/pool-adapter.js', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../../../src/core/cgdb/pool-adapter.js')>();
  return { ...actual, ...cgdbMocks };
});

vi.mock('../../../src/mcp/core/cgdb-adapter.js', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../../../src/mcp/core/cgdb-adapter.js')>();
  return { ...actual, ...cgdbMocks };
});
...
describe('LocalBackend @group repo routing', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'codragraph-atgrp-'));
