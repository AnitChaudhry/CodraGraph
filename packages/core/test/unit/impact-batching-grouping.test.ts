...
// Mock the cgdb-adapter module before importing LocalBackend so the class
// uses the mocked implementations of executeQuery / executeParameterized.
const executeQueryMock = vi.fn();
const executeParameterizedMock = vi.fn();

// Mock both the canonical source (core/cgdb/pool-adapter.js — what local-backend.ts
// imports) and the re-export shim (mcp/core/cgdb-adapter.js) so the mocks intercept
...
describe('impact: batching and grouping', () => {
  ...
  it('batches 250 IDs into 3 chunked STEP_IN_PROCESS queries', async () => {
    ...
    const repoHandle = {
      ...
      name: 'repo1',
      repoPath: '/tmp/repo',
      storagePath: '/tmp/repo/.codragraph',
      cgdbPath: '/tmp/repo/.codragraph/cgdb',
      indexedAt: 'now',
      lastCommit: 'c',
      stats: {},
    } as any;
  ...
  it('groups entry points across chunks and deduplicates correctly', async () => {
    ...
    const repoHandle = {
      ...
      name: 'repo2',
      repoPath: '/tmp/repo2',
      storagePath: '/tmp/repo2/.codragraph',
      cgdbPath: '/tmp/repo2/.codragraph/cgdb',
      indexedAt: 'now',
      lastCommit: 'c',
      stats: {},
    } as any;
  ...
  it('caps enrichment to MAX_CHUNKS and sets partial when capped', async () => {
    ...
    const repoHandle = {
      ...
      name: 'repo3',
      repoPath: '/tmp/repo3',
      storagePath: '/tmp/repo3/.codragraph',
      cgdbPath: '/tmp/repo3/.codragraph/cgdb',
      indexedAt: 'now',
      lastCommit: 'c',
      stats: {},
    } as any;
