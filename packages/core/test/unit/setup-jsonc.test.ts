...
describe('setupOpenCode â€” JSONC preservation', () => {
  ...
  "model": "test"
    ...
    expect(config.mcp.codragraph).toBeDefined();
  ...
  it('updates existing codragraph MCP entry without losing other keys', async () => {
  ...
  "mcp": {
    ...
    expect(config.mcp.codragraph).toEqual({
      type: 'local',
      command: ['/usr/local/bin/codragraph', 'mcp'],
    });
  });
  ...
  "mcp": {}
    ...
    expect(config.mcp.codragraph).toEqual({
      type: 'local',
      command: ['npx', '-y', '@codragraph/cli@2.2.0-rc.5', 'mcp'],
    });
  });
  ...
  "mcp": {}
    ...
    expect(config.mcp.codragraph).toEqual({
      type: 'local',
      command: ['bunx', '@codragraph/cli@2.2.0-rc.5', 'mcp'],
    });
  });
