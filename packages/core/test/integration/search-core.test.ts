 ...
 *
 * Tests: searchFTSFromCgdb via core adapter (no repoId) path against
 * indexed test data. Verifies ranked result ordering, score merging,
 * and empty-match behavior.
 *
 * Uses withTestCgdbDB wrapper for full lifecycle management.
 */
...
withTestCgdbDB(
  'search-core',
  (_handle) => {
    describe('searchFTSFromCgdb — core adapter (no repoId)', () => {
      it('returns ranked results for a matching query', async () => {
        const results = await searchFTSFromCgdb('user authentication', 10);
      ...
      it('merges scores from multiple node types for the same filePath', async () => {
        const results = await searchFTSFromCgdb('user authentication', 20);
      ...
      it('respects limit parameter', async () => {
        const results = await searchFTSFromCgdb('user authentication', 2);
        expect(results.length).toBeLessThanOrEqual(2);
      });

      it('returns empty array for a non-matching query', async () => {
        const results = await searchFTSFromCgdb('xyzzyplughtwisty', 10);
    ...
    describe('unhappy paths', () => {
      it('returns empty array for empty query string', async () => {
        const results = await searchFTSFromCgdb('', 10);
        expect(results).toEqual([]);
      });
      ...
      it('handles special characters in query gracefully', async () => {
        const results = await searchFTSFromCgdb('user* OR auth+', 10);
        expect(Array.isArray(results)).toBe(true);
      });

      it('handles limit of 0', async () => {
        const results = await searchFTSFromCgdb('user authentication', 0);
        expect(results).toEqual([]);
      });

      it('handles negative limit gracefully', async () => {
        const results = await searchFTSFromCgdb('user authentication', -1);
        expect(Array.isArray(results)).toBe(true);
      });

      it('handles very large limit', async () => {
        const results = await searchFTSFromCgdb('user authentication', 100000);
