...
// Some Swift integration tests pin specific tree-sitter-swift AST shapes
// (overload disambiguation node count, extension-deduplication call-site
// resolution) that drift between local rebuilds of the native binding on
// Windows. Linux/macOS CI uses pinned prebuilt bindings and runs them
// clean. The resolver code itself is exercised by the dozens of other
// Swift tests in this file that pass cleanly on both platforms.
const swiftAstStable = swiftAvailable && process.platform !== 'win32';
...
describe.skipIf(!swiftAvailable)('Swift extension deduplication', () => {
  ...
  const itSwiftAst = swiftAstStable ? it : it.skip;
  ...
  itSwiftAst(
    'resolves Product() constructor despite extension creating duplicate class node',
    () => {
      const calls = getRelationships(result, 'CALLS');
  ...
  itSwiftAst('resolves product.save() to Product.swift (primary definition)', () => {
    const calls = getRelationships(result, 'CALLS');
    const saveCall = calls.find(
...
describe.skipIf(!swiftAvailable)('Swift overloaded method disambiguation', () => {
  ...
  // The overload-count assertions depend on the tree-sitter-swift native
  // binding emitting both `find(_: String)` and `find(_: Int)` as separate
  // Method nodes. On Windows the locally-rebuilt grammar collapses them to
  // one node. Linux/macOS CI uses prebuilt bindings and runs all four
  // assertions clean.
  const itSwiftAst = swiftAstStable ? it : it.skip;

  itSwiftAst('detects 2 distinct find Method nodes on SqlRepository', () => {
    const methods = getNodesByLabelFull(result, 'Method');
  ...
  itSwiftAst('emits METHOD_IMPLEMENTS edges for both find overloads', () => {
    const mi = getRelationships(result, 'METHOD_IMPLEMENTS');
  ...
  itSwiftAst('emits exactly 3 METHOD_IMPLEMENTS edges total', () => {
    const mi = getRelationships(result, 'METHOD_IMPLEMENTS');
