 ...
 *
 * Tests: searchFTSFromCgdb via MCP pool adapter (with repoId) path
...
withTestCgdbDB(
  'search-pool',
  (handle) => {
    describe('searchFTSFromCgdb — MCP pool adapter (with repoId)', () => {
      it('returns ranked results via pool adapter', async () => {
        const results = await searchFTSFromCgdb('user authentication', 10, handle.repoId);
      ...
      it('returns empty array for non-matching query via pool adapter', async () => {
        const results = await searchFTSFromCgdb('xyzzyplughtwisty', 10, handle.repoId);
        expect(results).toEqual([]);
      });

      it('respects limit parameter via pool adapter', async () => {
        const results = await searchFTSFromCgdb('user authentication', 1, handle.repoId);
    ...
    describe('unhappy paths', () => {
      it('returns empty array for empty query via pool', async () => {
        const results = await searchFTSFromCgdb('', 10, handle.repoId);
        expect(results).toEqual([]);
      });
      ...
      it('handles special characters in query via pool', async () => {
        const results = await searchFTSFromCgdb('user* OR auth+', 10, handle.repoId);
        expect(Array.isArray(results)).toBe(true);
      });

      it('handles limit of 0 via pool', async () => {
        const results = await searchFTSFromCgdb('user authentication', 0, handle.repoId);
        expect(results).toEqual([]);
      });
    });
  },
