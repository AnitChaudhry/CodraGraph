/**
 * Integration Tests: cgdb-adapter busy/lock retry logic
 *
 * Tests isDbBusyError() detection and withCgdbDb() retry behaviour
 * using the real LadybugDB via withTestCgdbDB lifecycle.
 *
 * Follows existing cgdb integration test patterns (cgdb-core-adapter,
 * cgdb-pool, cgdb-pool-stability).
 */
...
import { isDbBusyError } from '../../src/core/cgdb/cgdb-adapter.js';
...
// ─── withCgdbDb retry integration tests ───────────────────────────────────

withTestCgdbDB('lock-retry', (handle) => {
  describe('withCgdbDb retry behaviour', () => {
    it('returns the operation result on success', async () => {
      const { withCgdbDb } = await import('../../src/core/cgdb/cgdb-adapter.js');
      const result = await withCgdbDb(handle.dbPath, async () => 'ok');
    ...
    it('retries on BUSY error and succeeds on later attempt', async () => {
      const { withCgdbDb } = await import('../../src/core/cgdb/cgdb-adapter.js');
      let callCount = 0;
      const result = await withCgdbDb(handle.dbPath, async () => {
        callCount++;
    ...
    it('propagates non-BUSY errors immediately without retrying', async () => {
      ...
      let callCount = 0;
      await expect(
        withCgdbDb(handle.dbPath, async () => {
          callCount++;
