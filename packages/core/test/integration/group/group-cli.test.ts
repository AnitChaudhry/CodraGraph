/**
 * Smoke-test `codragraph group` CLI via tsx (same pattern as cli-e2e.test.ts).
...
beforeAll(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'codragraph-group-cli-'));
});
...
function runGroup(args: string[]) {
  return spawnSync(process.execPath, ['--import', tsxImportUrl, cliEntry, 'group', ...args], {
    ...
    env: { ...process.env, CODRAGRAPH_HOME: tmpHome },
  });
}
...
describe('group CLI', () => {
  ...
  it('test_sync_command_source_does_not_call_blanket_closeCgdb', () => {
    const cliGroupPath = path.join(repoRoot, 'src', 'cli', 'group.ts');
    const source = fs.readFileSync(cliGroupPath, 'utf-8');

    // closeCgdb() without arguments (blanket close) must not appear.
    // Match closeCgdb() but not closeCgdb(someArg)
    const blanketClosePattern = /closeCgdb\s*\(\s*\)/;
  ...
  it('group impact runs with Issue #794 style flags (fixture-backed home)', () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), 'codragraph-cli-impact-'));
    try {
      ...
      const r = spawnSync(
        ...
        ],
        {
          ...
          timeout: 20000,
          stdio: ['pipe', 'pipe', 'pipe'],
          env: { ...process.env, CODRAGRAPH_HOME: home },
        },
      );
      expect(r.status).not.toBe(0);
      const msg = `${r.stderr}\n${r.stdout}`;
