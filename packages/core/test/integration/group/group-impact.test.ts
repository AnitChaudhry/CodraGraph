...
beforeAll(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'codragraph-grp-impact-int-'));
...
describe('group impact integration', () => {
  ...
  it('runs happy-path stub against fixture group (stops before bridge when no symbol UIDs)', async () => {
    const prev = process.env.CODRAGRAPH_HOME;
    process.env.CODRAGRAPH_HOME = tmpHome;
    try {
      const svc = new GroupService(stubPort());
      ...
      if (prev === undefined) delete process.env.CODRAGRAPH_HOME;
      else process.env.CODRAGRAPH_HOME = prev;
    }
  });
});
