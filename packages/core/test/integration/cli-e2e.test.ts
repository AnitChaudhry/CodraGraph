 ...
 * - analyzeCommand: verify pipeline runs and creates .codragraph/ output
 *
...
// Spawn the COMPILED CLI (`dist/cli/index.js`), not the tsx-loaded source.
// tsx + the LadybugDB native binding has a teardown interaction on Windows
// that segfaults the subprocess on exit even after the close-discipline fix
// — separate from the close-order bug fixed in `cgdb-adapter.ts`. Running
// the dist build matches what end-users actually do (`npx @codragraph/cli
// analyze .`) and exits cleanly. `beforeAll` builds dist if needed.
...
beforeAll(() => {
  ...
  // source — see `cliEntry` comment). Build only when missing so the
  // dev's normal incremental dist isn't clobbered on every test run.
  if (!fs.existsSync(cliEntry)) {
    execFileSync(process.execPath, [path.join(repoRoot, 'scripts', 'build.js')], {
      cwd: repoRoot,
      stdio: 'inherit',
    });
  }
...
/**
 * Auto-append `--no-setup` when the command is `analyze`. Without this the
 * first run on a clean test machine writes 32 skill files into the user's
 * `~/.claude/skills/`, `~/.cursor/skills/`, `~/.opencode/skill/`,
 * `~/.agents/skills/` plus MCP wiring into `~/.claude/`, `~/.codex/config.toml`,
 * etc. — pollution that survives the test, breaks repeated runs, and isn't
 * what these tests are exercising. The dedicated setup tests in
 * `test/integration/setup.test.ts` cover the first-run path explicitly.
 */
function withNoSetup(args: string[]): string[] {
  if (args[0] === 'analyze' && !args.includes('--no-setup')) {
    return [...args, '--no-setup'];
  }
  return args;
}

function runCli(command: string, cwd: string, timeoutMs = 15000) {
  return spawnSync(process.execPath, [cliEntry, ...withNoSetup([command])], {
    cwd,
    encoding: 'utf8',
    timeout: timeoutMs,
...
function runCliRaw(extraArgs: string[], cwd: string, timeoutMs = 15000) {
  return spawnSync(process.execPath, [cliEntry, ...withNoSetup(extraArgs)], {
    cwd,
    encoding: 'utf8',
    timeout: timeoutMs,
    stdio: ['pipe', 'pipe', 'pipe'],
 ...
 * isolate the global registry via CODRAGRAPH_HOME so they don't touch the
 * developer / CI agent's real ~/.codragraph/registry.json (#829).
 */
...
describe('CLI end-to-end', () => {
  ...
  it('analyze command runs pipeline on mini-repo', () => {
    ...
    // Successful analyze should create .codragraph/ output directory
    const codragraphDir = path.join(MINI_REPO, '.codragraph');
    expect(fs.existsSync(codragraphDir)).toBe(true);
    expect(fs.statSync(codragraphDir).isDirectory()).toBe(true);
  }, 60_000);

  // ─── analyze --name <alias> + --allow-duplicate-name (#829) ──────
  //
  // End-to-end regression guard for the name-collision feature:
  //   1. `analyze --name X` persists the alias to ~/.codragraph/registry.json
  ...
  describe('remove <target> (#664)', () => {
    ...
    it('refuses to proceed when a registry entry points storagePath outside <repo>/.codragraph (#1003)', () => {
      // Regression guard for the safety gap flagged by @magyargergo on
      // PR #1003: `~/.codragraph/registry.json` is a user-writable JSON
      ...
      // `<entry.path>/.codragraph`. We verify:
      //   1. Exit code 1 with the actionable "registry entry corrupted"
      //      hint.
      //   2. The .codragraph/ storage dir is UNTOUCHED.
      ...
      try {
        ...
        expect(fs.existsSync(path.join(repo, '.codragraph'))).toBe(true);
        ...
        // Repo + .codragraph dir + .git dir must all still exist — the
  ...
  // the rest. We verify:
  //   1. The poisoned entry is NOT deleted (working tree + .codragraph
  ...
  //      .codragraph dir AND its registry entry are gone).
  describe('clean --all with a poisoned registry entry (#1003)', () => {
    it('skips poisoned entries, cleans valid ones, never deletes the working tree', () => {
      ...
      try {
        ...
        // Sanity: both working trees and .codragraph dirs still exist.
        expect(fs.existsSync(repoBad)).toBe(true);
        expect(fs.existsSync(path.join(repoBad, '.codragraph'))).toBe(true);
        expect(fs.existsSync(path.join(repoBad, '.git'))).toBe(true);
        expect(fs.existsSync(path.join(repoGood, '.codragraph'))).toBe(true);
        ...
        // Poisoned repo: working tree + .codragraph + .git all SURVIVE.
        expect(fs.existsSync(repoBad), 'poisoned repo working tree must survive').toBe(true);
        expect(
          fs.existsSync(path.join(repoBad, '.codragraph')),
          'poisoned repo .codragraph must survive (guard refused to rm repo root)',
        ).toBe(true);
        expect(fs.existsSync(path.join(repoBad, '.git')), '.git must survive').toBe(true);

        // Good repo: its .codragraph IS gone (cleanup succeeded despite
        // the poisoned sibling entry — per-entry error tolerance is
        // preserved).
        expect(
          fs.existsSync(path.join(repoGood, '.codragraph')),
          'good repo .codragraph should be cleaned',
        ).toBe(false);
        // But the good repo's working tree stays (clean never touches
        // anything outside .codragraph).
  ...
  describe('unhappy path', () => {
    ...
    it('shows help with --help flag', () => {
      ...
      expect(result.stdout).toMatch(/codragraph/i);
  ...
  describe('CLI error handling', () => {
     ...
     * Spawns the compiled `dist/cli/index.js` so the test process doesn't
     * need a TS loader at the foreign cwd.
     */
    ...
    it('status on non-indexed repo reports not indexed', () => {
      ...
      // in this suite may have created MINI_REPO/.codragraph via analyze,
      // and findRepo() walks up so any `.codragraph` along the path still
  ...
  describe('wiki command flags', () => {
    ...
    it('wiki on non-indexed repo fails with "No CodraGraph index"', () => {
      ...
      try {
        ...
        // Must spawn outside project tree so it doesn't find parent .codragraph
        const result = spawnSync(process.execPath, [cliEntry, 'wiki', tmpDir], {
          cwd: tmpDir,
          encoding: 'utf8',
          timeout: 15000,
          stdio: ['pipe', 'pipe', 'pipe'],
        ...
        expect(result.status).toBe(1);
        expect(result.stdout).toMatch(/No CodraGraph index found/);
      } finally {
        fs.rmSync(tmpDir, { recursive: true, force: true });
      }
    });
  ...
  // Requires analyze to have run first (the analyze test above populates .codragraph/).
  ...
  describe('EPIPE handling (#324)', () => {
    it('cypher: EPIPE exits with code 0, not stderr dump', () => {
      return new Promise<void>((resolve, reject) => {
        const child = spawn(
          process.execPath,
          [cliEntry, 'cypher', 'MATCH (n) RETURN n LIMIT 500', '--repo', 'mini-repo'],
          {
            cwd: MINI_REPO,
  ...
  describe('eval-server READY signal (#324)', () => {
    it('READY signal appears on stdout, not stderr', () => {
      return new Promise<void>((resolve, reject) => {
        const child = spawn(
          process.execPath,
          [cliEntry, 'eval-server', '--port', '0', '--idle-timeout', '3'],
          {
            cwd: MINI_REPO,
            stdio: ['ignore', 'pipe', 'pipe'],
            env: {
        ...
        child.stdout.on('data', (chunk: Buffer) => {
          ...
          if (stdoutBuffer.includes('CODRAGRAPH_EVAL_SERVER_READY:')) {
            foundOnStdout = true;
            child.kill('SIGTERM');
          }
        });
        ...
        child.stderr.on('data', (chunk: Buffer) => {
          ...
          if (text.includes('CODRAGRAPH_EVAL_SERVER_READY:')) {
            foundOnStderr = true;
            child.kill('SIGTERM');
          }
        });
