 ...
 *   - Concurrent initCgdb deduplication
...
withTestCgdbDB(
  ...
  (handle) => {
    ...
    const ensurePool = async () => {
      if (!inited) {
        await initCgdb(REPO, handle.dbPath);
        inited = true;
      }
    };
    ...
    it('concurrent initCgdb calls for the same repoId deduplicate', async () => {
      const promises = Array.from({ length: 6 }, () => initCgdb('dedup-repo', handle.dbPath));
      await Promise.all(promises);
      expect(isCgdbReady('dedup-repo')).toBe(true);
      const rows = await executeQuery('dedup-repo', 'MATCH (n:Function) RETURN n.name AS name');
      expect(rows.length).toBeGreaterThanOrEqual(2);
      await closeCgdb('dedup-repo');
    });

    it('pool entry is not visible until initCgdb fully resolves', async () => {
      const initPromise = initCgdb('atomic-repo', handle.dbPath);
      // Synchronously, pool should NOT be ready (pool.set is the last operation)
      expect(isCgdbReady('atomic-repo')).toBe(false);
      await initPromise;
      expect(isCgdbReady('atomic-repo')).toBe(true);
      await closeCgdb('atomic-repo');
    });
  },
  {
    seed: SEED_DATA,
