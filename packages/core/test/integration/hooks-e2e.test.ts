 ...
 *
 * Tests the hook scripts with real git repos and .codragraph directories.
...
describe.each(HOOKS)('hooks e2e ($name)', ({ path: hookPath }) => {
  ...
  describe('PreToolUse — silent without codragraph CLI', () => {
    // PreToolUse tries to spawn `codragraph augment` which won't be available in CI.
    ...
    it('handles Grep pattern gracefully when CLI is unavailable', () => {
      ...
      // spawned `codragraph augment` hangs and the 10s timeout kills the process.
  ...
  describe('unhappy paths', () => {
    it('handles corrupted meta.json (invalid JSON) without crashing', () => {
      fs.writeFileSync(path.join(codragraphDir, 'meta.json'), 'THIS IS NOT JSON {{{');
    ...
    it('handles meta.json with missing lastCommit field', () => {
      fs.writeFileSync(path.join(codragraphDir, 'meta.json'), JSON.stringify({ stats: {} }));
  ...
  describe('directory without .codragraph', () => {
    // The hook walks up 5 parent directories looking for .codragraph.
    // To guarantee none is found, create a deeply nested temp dir at the
    // filesystem root where no .codragraph could exist in any ancestor.
    let noCodraGraphDir: string;

    beforeAll(() => {
      // Use a root-level temp path so parent traversal can't find .codragraph
      const root = os.platform() === 'win32' ? 'C:\\' : '/tmp';
      const base = path.join(root, `no-codragraph-${Date.now()}`);
      // Nest 6 levels deep (hook walks up 5) to ensure isolation
      noCodraGraphDir = path.join(base, 'a', 'b', 'c', 'd', 'e', 'f');
      fs.mkdirSync(noCodraGraphDir, { recursive: true });
      spawnSync('git', ['init'], { cwd: noCodraGraphDir, stdio: 'pipe' });
    });
    ...
    afterAll(() => {
      ...
      const base = path.join(
        root,
        path.basename(path.resolve(noCodraGraphDir, '..', '..', '..', '..', '..', '..')),
      );
      fs.rmSync(base, { recursive: true, force: true });
    });

    it('ignores PostToolUse when no .codragraph directory exists', () => {
      const result = runHook(hookPath, {
        hook_event_name: 'PostToolUse',
        tool_name: 'Bash',
        tool_input: { command: 'git commit -m "x"' },
    ...
    it('ignores PreToolUse when no .codragraph directory exists', () => {
      const result = runHook(hookPath, {
        hook_event_name: 'PreToolUse',
        tool_name: 'Grep',
