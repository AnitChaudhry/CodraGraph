...
import { NODE_TABLES, EMBEDDING_TABLE_NAME } from '../../src/core/cgdb/schema.js';
...
/** FTS index definition for withTestCgdbDB */
...
/**
 * Options for withTestCgdbDB lifecycle.
 *
 * Lifecycle: initCgdb → loadFTS → dropFTS → clearData → seed
 *            → createFTS → [closeCoreCgdb + poolInitCgdb] → afterSetup
 */
export interface WithTestCgdbDBOptions {
 ...
 * no raw cgdb.Database() connections are opened.  This avoids file-lock
 ...
 */
export function withTestCgdbDB(
  prefix: string,
  fn: (handle: IndexedDBHandle) => void,
  options?: WithTestCgdbDBOptions,
): void {
  ...
  const setup = async () => {
    ...
    const dbPath = inject<'cgdbDbPath'>('cgdbDbPath');
    ...
    if (options?.poolAdapter) {
      ...
      if (!coreDb) throw new Error('withTestCgdbDB: core adapter has no open Database');
      const { initCgdbWithDb } = await import('../../src/core/cgdb/pool-adapter.js');
      await initCgdbWithDb(repoId, coreDb, dbPath);
    }

    const cleanup = async () => {
      if (options?.poolAdapter) {
        const poolAdapter = await import('../../src/core/cgdb/pool-adapter.js');
        await poolAdapter.closeCgdb(repoId);
      }
      await adapter.closeCgdb();
    };

    // tmpHandle.dbPath → parent temp dir (not the cgdb file) so tests
  ...
  const lazyHandle = new Proxy({} as IndexedDBHandle, {
    get(_target, prop) {
      if (!ref.handle)
        throw new Error('withTestCgdbDB: handle not initialized — beforeAll has not run yet');
      return (ref.handle as any)[prop];
    },
  });

  // Wrap in describe to scope beforeAll/afterAll — prevents lifecycle
  // collisions when multiple withTestCgdbDB calls share the same file.
  describe(`withTestCgdbDB(${prefix})`, () => {
