...
import {
  ...
  closeCgdb,
  withCgdbDb,
} from '../core/cgdb/cgdb-adapter.js';
...
export const createServer = async (port: number, host: string = '127.0.0.1') => {
  ...
  const importRecipeHandler = async (
    ...
    try {
      ...
      throw new Error(
        `recipes endpoint requires codragraph-harness to be installed: ${err?.message ?? String(err)}`,
      );
    }
  };
  ...
  app.get('/api/groups', async (_req, res) => {
    try {
      const result = (await getGroupSvc().groupList({})) as { groups?: string[] };
  ...
  app.get('/api/groups/for-repo', async (req, res) => {
    try {
      ...
      const list = (await svc.groupList({})) as { groups?: string[] };
      const matches: Array<{ group: string; groupPath: string }> = [];
      for (const groupName of list.groups ?? []) {
        const details = (await svc.groupList({ name: groupName })) as {
          name?: string;
          repos?: Record<string, string>;
        };
        if (!details.repos) continue;
  ...
  app.get('/api/groups/:name/contracts', async (req, res) => {
    try {
      const params: Record<string, unknown> = { name: req.params.name };
      if (typeof req.query.type === 'string') params.type = req.query.type;
      if (typeof req.query.repo === 'string') params.repo = req.query.repo;
      if (req.query.unmatchedOnly === 'true') params.unmatchedOnly = true;
  ...
  app.get('/api/graph', async (req, res) => {
    try {
      ...
      const isCgdbMaterialized = await (async (): Promise<boolean> => {
        try {
  ...
  app.get('/api/grep', async (req, res) => {
    try {
      ...
      const cgdbPath = path.join(entry.storagePath, 'cgdb');
      const fileRows = await withCgdbDb(cgdbPath, () =>
