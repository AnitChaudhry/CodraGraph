...
import {
  ...
  isCgdbReady,
  isWriteQuery,
} from '../../core/cgdb/pool-adapter.js';
export { isWriteQuery };
...
function logQueryTiming(query: string, phases: Record<string, number>): void {
  ...
  console.error(
    `CodraGraph [query:timing] query=${JSON.stringify(truncated)} totalMs=${totalMs} phases=${JSON.stringify(phases)}`,
  );
}
...
export class LocalBackend {
  ...
  private async refreshRepos(): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      if (kuzu.found && kuzu.needsReindex) {
        console.error(
          `CodraGraph: "${entry.name}" has a stale KuzuDB index. Run: codragraph analyze ${entry.path}`,
        );
      }
  ...
  async callTool(method: string, params: any): Promise<any> {
    ...
    switch (method) {
      ...
      case 'harness_swarm_run': {
        ...
        try {
          const mod = (await dynImport(harnessModuleId)) as {
            handleHarnessSwarmRun?: (p: unknown) => Promise<unknown>;
          };
      ...
      case 'graphstore_blame_symbol': {
        ...
        switch (method) {
          ...
          case 'graphstore_merge': {
            ...
            if (!source) throw new Error('graphstore_merge requires `source`');
      ...
      case 'harness_recipes_lookup': {
        ...
        try {
          mod = (await dynImport(harnessModuleId)) as typeof mod;
        } catch (err: unknown) {
          throw new Error(
            `${method} requires the \`codragraph-harness\` package. ` +
              `Underlying error: ${err instanceof Error ? err.message : String(err)}`,
          );
        }
        const handler =
          method === 'harness_recipes_list'
      ...
      case 'harness_run': {
        ...
        try {
          const mod = (await dynImport(harnessModuleId)) as {
            handleHarnessRun?: (p: unknown) => Promise<unknown>;
          };
  ...
  private async query(
    ...
    return {
      ...
      ...(!ftsUsed && {
        warning:
          'FTS extension unavailable - keyword search degraded. Run: codragraph analyze --force to rebuild indexes.',
      }),
    };
  }
