...
import { encodeContent, type ContentEncoding } from '@codragraph/graphstore';
...
const applyEncoding = (
  content: string,
  compress: ContentEncoding | undefined,
): { wireContent: string; tag: string } => {
...
export const streamAllCSVsToDisk = async (
  ...
  const fileWriter = new BufferedCSVWriter(
    ...
    'id,name,filePath,content,contentEncoding',
  );
  const folderWriter = new BufferedCSVWriter(path.join(csvDir, 'folder.csv'), 'id,name,filePath');
  const codeElementHeader =
    'id,name,filePath,startLine,endLine,isExported,content,contentEncoding,description';
  ...
  const methodHeader =
    'id,name,filePath,startLine,endLine,isExported,content,contentEncoding,description,parameterCount,returnType';
  ...
  const sectionWriter = new BufferedCSVWriter(
    ...
    'id,name,filePath,startLine,endLine,level,content,contentEncoding,description',
  );
