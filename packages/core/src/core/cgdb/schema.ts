...
import { NODE_TABLES, REL_TABLE_NAME, REL_TYPES, EMBEDDING_TABLE_NAME } from '@codragraph/shared';
...
export type { NodeTableName, RelType } from '@codragraph/shared';
...
const _rawDims = parseInt(process.env.CODRAGRAPH_EMBEDDING_DIMS ?? '384', 10);
if (Number.isNaN(_rawDims) || _rawDims <= 0) {
  throw new Error(
    `CODRAGRAPH_EMBEDDING_DIMS must be a positive integer, got "${process.env.CODRAGRAPH_EMBEDDING_DIMS}"`,
  );
}
