...
let db: cgdb.Database | null = null;
let conn: cgdb.Connection | null = null;
...
export const getDatabase = (): cgdb.Database | null => db;
...
export const initCgdb = async (dbPath: string) => {
 ...
 */
export const withCgdbDb = async <T>(dbPath: string, operation: () => Promise<T>): Promise<T> => {
...
const ensureCgdbInitialized = async (dbPath: string) => {
...
const doInitCgdb = async (dbPath: string) => {
...
export const loadGraphToCgdb = async (
 ...
 */
export const insertNodeToCgdb = async (
  label: string,
 ...
 */
export const batchInsertNodesToCgdb = async (
 ...
 */
const liveCgdbHandles = new Set<unknown>();

const closeQueryResult = async (qr: unknown): Promise<void> => {
  if (!qr) return;
  ...
  for (const r of candidates) {
    ...
    try {
      ...
      if (typeof close === 'function') await Promise.resolve(close.call(r));
    } catch {
      /* best-effort */
    }
  }
...
const closeStmt = async (stmt: unknown): Promise<void> => {
  if (!stmt) return;
  liveCgdbHandles.delete(stmt);
  try {
    const close = (stmt as { close?: () => unknown })?.close;
    if (typeof close === 'function') await Promise.resolve(close.call(stmt));
  } catch {
    /* best-effort */
  }
};
 ...
 */
const trackConnection = (rawConn: cgdb.Connection): cgdb.Connection => {
  const c = rawConn as unknown as Record<string, any> & { __cgdbTracked?: boolean };
  if (c.__cgdbTracked) return rawConn;
  c.__cgdbTracked = true;
  const wrap = (orig: any) =>
    async function (this: unknown, ...args: unknown[]) {
      const qr = await orig.apply(this, args);
      if (qr && typeof qr === 'object') {
        if (Array.isArray(qr)) qr.forEach((r) => r && liveCgdbHandles.add(r));
        else liveCgdbHandles.add(qr);
      }
      return qr;
    };
  if (typeof c.query === 'function') c.query = wrap(c.query);
  if (typeof c.execute === 'function') c.execute = wrap(c.execute);
  if (typeof c.prepare === 'function') c.prepare = wrap(c.prepare);
  return rawConn;
};
...
export const getCgdbStats = async (): Promise<{ nodes: number; edges: number }> => {
...
export const closeCgdb = async (): Promise<void> => {
  ...
  for (const h of handles) {
    try {
      const close = (h as { close?: () => unknown })?.close;
      if (typeof close === 'function') await Promise.resolve(close.call(h));
    } catch {
      /* best-effort */
    }
  }
...
export const isCgdbReady = (): boolean => conn !== null && db !== null;
...
export const deleteNodesForFile = async (
  ...
  let tempDb: cgdb.Database | null = null;
  let tempConn: cgdb.Connection | null = null;
  let targetConn: cgdb.Connection | null = conn;
