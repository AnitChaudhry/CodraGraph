...
export function getDefaultCodragraphDir(): string {
  return process.env.CODRAGRAPH_HOME || path.join(os.homedir(), '.codragraph');
}

export function getGroupsBaseDir(codragraphDir?: string): string {
  return path.join(codragraphDir || getDefaultCodragraphDir(), 'groups');
}
...
export function getGroupDir(codragraphDir: string, groupName: string): string {
  validateGroupName(groupName);
  return path.join(codragraphDir, 'groups', groupName);
}
...
export async function listGroups(codragraphDir?: string): Promise<string[]> {
  const groupsDir = getGroupsBaseDir(codragraphDir);
  try {
...
export async function createGroupDir(
  codragraphDir: string,
  groupName: string,
  force: boolean = false,
): Promise<string> {
  const groupDir = getGroupDir(codragraphDir, groupName);
