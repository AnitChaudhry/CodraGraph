...
export async function ensureBridgeSchema(handle: BridgeHandle): Promise<void> {
  const conn = handle._conn as cgdb.Connection;
  for (const q of BRIDGE_SCHEMA_QUERIES) {
    try {
      await conn.query(q);
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      if (!msg.includes(CGDB_ALREADY_EXISTS_MSG)) throw err;
    }
  }
}
 ...
 */
async function closeBridgeHandle(h: unknown): Promise<void> {
  if (!h) return;
 ...
 */
function isTransientCgdbLockError(err: unknown): boolean {
...
async function queryBridgeOnce<T>(
  handle: BridgeHandle,
  cypher: string,
  params?: Record<string, CgdbValue>,
): Promise<T[]> {
  const conn = handle._conn as cgdb.Connection;
...
export async function queryBridge<T>(
  ...
  for (let attempt = 0; attempt < ATTEMPTS; attempt++) {
    try {
      return await queryBridgeOnce<T>(handle, cypher, params);
    } catch (err) {
      if (!isTransientCgdbLockError(err) || attempt === ATTEMPTS - 1) throw err;
 ...
 */
function unwrapQueryResult(queryResult: cgdb.QueryResult | cgdb.QueryResult[]): cgdb.QueryResult {
...
export async function closeBridgeDb(handle: BridgeHandle): Promise<void> {
  try {
    await (handle._conn as cgdb.Connection).close();
  } catch {
    /* ignore */
  }
  try {
    await (handle._db as cgdb.Database).close();
  } catch {
    /* ignore */
  }
}
...
export async function openBridgeDbReadOnly(groupDir: string): Promise<BridgeHandle | null> {
  ...
  for (let attempt = 0; attempt < ATTEMPTS; attempt++) {
    ...
    try {
      ...
      if (!isTransientCgdbLockError(err) || attempt === ATTEMPTS - 1) return null;
      await new Promise((r) => setTimeout(r, 50 * Math.pow(2, attempt)));
      continue;
    }
  }
