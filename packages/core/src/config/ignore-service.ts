...
// hardcoded rule via a `!pattern` line in `.codragraphignore` (#771) — same
...
/**
 * Load .gitignore and .codragraphignore rules from the repo root.
 * Returns an `ignore` instance with all patterns, or null if no files found.
 */
export interface IgnoreOptions {
  /** Skip .gitignore parsing, only read .codragraphignore. Defaults to CODRAGRAPH_NO_GITIGNORE env var. */
  noGitignore?: boolean;
}
...
export const loadIgnoreRules = async (
  ...
  const skipGitignore = options?.noGitignore ?? !!process.env.CODRAGRAPH_NO_GITIGNORE;
  const filenames = skipGitignore ? ['.codragraphignore'] : ['.gitignore', '.codragraphignore'];
...
/**
 * Walk ancestor segments of `rel` and check whether `.codragraphignore`
 ...
 * the user has an explicit `!__tests__/` line in `.codragraphignore`.
 ...
 * - .gitignore / .codragraphignore patterns (via `ignore` package)
 ...
 *
 * Precedence (#771): user's `.codragraphignore` negation patterns take
...
export const createIgnoreFilter = async (repoPath: string, options?: IgnoreOptions) => {
  ...
  return {
    ignored(p: Path): boolean {
      ...
      if (!rel) return false;
      // User's .codragraphignore negation takes precedence over hardcoded
      ...
      // Check .gitignore / .codragraphignore patterns
    ...
    childrenIgnored(p: Path): boolean {
      ...
      const rel = p.relative();
      // User's .codragraphignore negation takes precedence (#771) — if the
      ...
      // Check against .gitignore / .codragraphignore patterns.
