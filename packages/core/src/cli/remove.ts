 ...
 * Remove Command (#664)
 *
 * Delete the `.codragraph/` index for a registered repo and unregister it
 * from the global registry (~/.codragraph/registry.json). The target is
 ...
 *     .codragraph/ directories on disk.
...
export const removeCommand = async (target: string, options?: { force?: boolean }) => {
  ...
  if (!options?.force) {
    console.log(`This will delete the CodraGraph index for: ${entry.name}`);
  ...
  // `<entry.path>/.codragraph` subfolder. `~/.codragraph/registry.json` is
