 ...
 */
export const SKILL_TARGETS = ['claude', 'cursor', 'opencode', 'codex'] as const;
export type SkillTarget = (typeof SKILL_TARGETS)[number];

const SKILL_OUTPUT_DIRS: Record<SkillTarget, string[]> = {
...
export const generateSkillFiles = async (
  ...
  const effectiveTargets = targets.length > 0 ? targets : (['claude'] as SkillTarget[]);
...
const renderSkillMarkdown = (
  ...
  lines.push('');
  lines.push(`1. \`codragraph_context({name: "${firstEntry}"})\` \u2014 see callers and callees`);
  lines.push(
    `2. \`codragraph_query({query: "${community.label.toLowerCase()}"})\` \u2014 find related execution flows`,
  );
