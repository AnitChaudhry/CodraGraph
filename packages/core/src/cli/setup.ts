...
function mergeMcpConfig(existing: any): any {
  ...
  existing.mcpServers.codragraph = getMcpEntry();
  return existing;
}
...
/**
 * Install CodraGraph skills to ~/.claude/skills/ for Claude Code.
 */
...
/**
 * Install CodraGraph hooks to ~/.claude/settings.json for Claude Code.
...
async function installClaudeCodeHooks(result: SetupResult): Promise<void> {
  ...
  try {
    ...
    const dest = path.join(destHooksDir, 'codragraph-hook.cjs');
    try {
    ...
    const hookPath = path.join(destHooksDir, 'codragraph-hook.cjs').replace(/\\/g, '/');
    ...
    // Helper: add a hook entry if one with 'codragraph-hook' isn't already registered
    interface HookEntry {
    ...
    function ensureHookEntry(
      ...
      const hasHook = existing.hooks[eventName].some((h: HookEntry) =>
        h.hooks?.some((hh) => hh.command?.includes('codragraph-hook')),
      );
      if (!hasHook) {
...
function getCodexMcpTomlSection(): string {
  ...
  return `[mcp_servers.codragraph]\ncommand = ${command}\nargs = ${args}\n`;
}

/**
 * Append CodraGraph MCP server config to Codex's config.toml if missing.
 */
async function upsertCodexConfigToml(configPath: string): Promise<void> {
  ...
  if (existing.includes('[mcp_servers.codragraph]')) {
    return;
  }
...
/**
 * Install CodraGraph skills to a target directory.
 * Each skill is installed as {targetDir}/codragraph-{skillName}/SKILL.md
...
/**
 * Install global Cursor skills to ~/.cursor/skills/codragraph/
 */
...
/**
 * Install global OpenCode skills to ~/.config/opencode/skill/codragraph/
 */
...
/**
 * Install global Codex skills to ~/.agents/skills/codragraph/
 */
...
export const runSetup = async (options: RunSetupOptions = {}): Promise<SetupResult> => {
