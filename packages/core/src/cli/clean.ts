 ...
 * Clean Command
 *
 * Removes the .codragraph index from the current repository.
...
export const cleanCommand = async (options?: { force?: boolean; all?: boolean }) => {
  ...
  if (options?.all) {
    if (!options?.force) {
      ...
      console.log(`This will delete CodraGraph indexes for ${entries.length} repo(s):`);
    ...
    for (const entry of entries) {
      ...
      // remove.ts. `~/.codragraph/registry.json` is user-writable, so a
  ...
  if (!options?.force) {
    console.log(`This will delete the CodraGraph index for: ${repoName}`);
