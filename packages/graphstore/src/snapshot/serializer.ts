...
import { normalizeRowForHash } from './content-codec.js';
...
export const serializeSnapshot = async (
  ...
  for (const tableName of nodeTableNames) {
    ...
    for await (const row of source.streamNodeTable(tableName)) {
      ...
      // RFC 0001 Phase 1: hash the normalized (decoded, encoding-stripped)
      // row so a compressed row and its raw twin map to the same CAS id.
      // Identity-op in 1.7.x because no row carries contentEncoding yet.
      const rowId = await putJsonCounted(normalizeRowForHash(row));
  ...
  for await (const row of source.streamEdges()) {
    ...
    // RFC 0001 Phase 1 — see node-table loop above. Edges don't carry
    // `content` today, so this is doubly an identity op, but we keep
    // the call site symmetric so future schema changes flow through
...
/**
 * Pull the logical id out of a node row. Node tables in the cgdb schema
