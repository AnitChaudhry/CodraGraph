# RFC — packages/graphstore (Phase 4)
...
5. **Engine-agnostic core** — the graphstore package itself does not
   ...
   thin adapter inside `packages/core/`.
...
- The moat hook: versioned recipe memory in `packages/harness` keyed
...
`packages/core/src/core/run-analyze.ts` calls `recordAnalysisSnapshot` after
`loadGraphToCgdb` completes. The hook is **best-effort** — wrapped in
...
| Decision | Why |
|---|---|
| Hybrid storage (CAS + cgdb, not replace) | Preserves every existing query path; reversible if the experiment doesn't pan out |
...
| Single rel manifest (no per-pair split) | Phase 4 keeps the relationship table as one logical object; the existing cgdb per-label-pair CSV splitting is a load-time optimization, not a versioning concern |
...
| packages/graphstore depends on packages/shared, not codragraph | Avoids the circular import; shared has the table-name constants we need |
...
2. **CSV-based bulk loader for `materialize`** — current
   `CgdbRowSinkForCheckout` does per-row CREATE. Fine up to ~50k rows;
   beyond that, switch to the same CSV + COPY path that `loadGraphToCgdb`
   uses. Profile-guided.
3. **The moat hook (versioned recipe memory)** — store harness Pareto
   recipes keyed by `(snapshot_id, task_family, recipe_id)`. Belongs
   in `packages/harness` now that Phase 3 + Phase 4 are both live —
...
| 2   | Dashboard MVP in `apps/web`            | Pending (consumes Phase 4 data model) |
