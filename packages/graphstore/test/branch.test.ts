...
describe('branch refs', () => {
  it('createBranch writes a ref and refuses to overwrite', async () => {
    ...
    await expect(createBranch({ root: tmpRoot, name: 'main', commit: commitB })).rejects.toThrow();
  });
  ...
  it('rejects path-traversal branch names', async () => {
    ...
    await expect(createBranch({ root: tmpRoot, name: '/abs', commit: commitA })).rejects.toThrow(
      /Invalid branch name/,
    );
  });
});
