...
Tests how much semantic similarity is preserved in vector embeddings after codragraph compression.

**Methodology:**
1. Generate embeddings for original text using OpenAI's text-embedding model
2. Compress the text using codragraph compression
...
```bash
python benchmark/embedding_similarity/llm.py

# Specify output file
python benchmark/embedding_similarity/llm.py --output results.json

# Use different models
python benchmark/embedding_similarity/llm.py --compression-model gpt-4o-mini --embedding-model text-embedding-3-small
```

### NLP-based Compression

```bash
python benchmark/embedding_similarity/nlp.py

# Specify language
python benchmark/embedding_similarity/nlp.py --lang es
```
