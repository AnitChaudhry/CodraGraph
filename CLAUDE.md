...
Prefer **PreToolUse** hooks for hard gates (e.g. tests before `git_commit`). Adapt hook commands to `codragraph/` npm scripts.
...
- **Call-resolution DAG:** See ARCHITECTURE.md § Call-Resolution DAG. Shared pipeline code in `codragraph/src/core/ingestion/` must not name languages — use `LanguageProvider` hooks instead (see AGENTS.md).
