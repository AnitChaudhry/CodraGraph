...
CODRAGRAPH_COBOL_DIRS=s,c,wfproc CODRAGRAPH_VERBOSE=1 node --max-old-space-size=8192 \
  /path/to/codragraph/dist/cli/index.js analyze --force
```
...
```bash
node --max-old-space-size=16384 /path/to/codragraph/dist/cli/index.js analyze
```
...
- `codragraph/src/core/ingestion/workers/worker-pool.ts` -- `DEFAULT_SUB_BATCH_SIZE`, `SUB_BATCH_TIMEOUT_MS`, `WORKER_STARTUP_TIMEOUT_MS`
- `codragraph/src/core/ingestion/pipeline.ts` -- `CHUNK_BYTE_BUDGET`, COBOL sub-batch configuration, chunk lifecycle
- `codragraph/src/core/ingestion/workers/parse-worker.ts` -- `MAX_DATA_ITEMS_PER_FILE`, `processCobolRegexOnly()`
- `codragraph/src/core/ingestion/parsing-processor.ts` -- Sequential fallback `MAX_DATA_ITEMS_PER_FILE`
