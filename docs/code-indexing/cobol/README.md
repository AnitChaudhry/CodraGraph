...
CodraGraph indexes COBOL codebases using a **regex-only extraction** strategy, bypassing tree-sitter entirely. This document explains why, how the pipeline works, and links to detailed sub-documents.
...
|----------|-------------|
| [File Detection](./file-detection.md) | Extension mapping, `CODRAGRAPH_COBOL_DIRS`, copybook classification |
...
| File | Purpose |
|------|---------|
| `packages/core/src/core/ingestion/cobol-preprocessor.ts` | Patch marker cleanup + regex extraction engine |
| `packages/core/src/core/ingestion/cobol-copy-expander.ts` | COPY statement expansion with REPLACING |
| `packages/core/src/core/ingestion/utils.ts` | `getLanguageFromPath`, `getLanguageFromFilename` |
| `packages/core/src/core/ingestion/pipeline.ts` | `isCobolCopybook`, `expandCobolCopies`, `detectCrossProgamContracts` |
| `packages/core/src/core/ingestion/workers/parse-worker.ts` | `processCobolRegexOnly` -- graph model builder |
| `packages/core/src/core/ingestion/workers/worker-pool.ts` | Configurable sub-batch size for COBOL |
