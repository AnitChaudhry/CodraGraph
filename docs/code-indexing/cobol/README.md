...
| File | Purpose |
|------|---------|
| `codragraph/src/core/ingestion/cobol-preprocessor.ts` | Patch marker cleanup + regex extraction engine |
| `codragraph/src/core/ingestion/cobol-copy-expander.ts` | COPY statement expansion with REPLACING |
| `codragraph/src/core/ingestion/utils.ts` | `getLanguageFromPath`, `getLanguageFromFilename` |
| `codragraph/src/core/ingestion/pipeline.ts` | `isCobolCopybook`, `expandCobolCopies`, `detectCrossProgamContracts` |
| `codragraph/src/core/ingestion/workers/parse-worker.ts` | `processCobolRegexOnly` -- graph model builder |
| `codragraph/src/core/ingestion/workers/worker-pool.ts` | Configurable sub-batch size for COBOL |
