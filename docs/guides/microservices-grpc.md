...
```bash
npx @codragraph/cli group sync payments-platform --verbose
```

What this does (see [`sync.ts`](../../packages/core/src/core/group/sync.ts)):

1. Opens each member's per-repo LadybugDB.
2. Runs the HTTP, gRPC, and topic extractors against the source files.
3. Applies manifest `links` through [`manifest-extractor.ts`](../../packages/core/src/core/group/extractors/manifest-extractor.ts).
...
```bash
npx @codragraph/cli group contracts payments-platform --type grpc --json
```
...
Staleness of the underlying indexes shows up in `npx @codragraph/cli group status payments-platform` or the `codragraph://group/<name>/status` resource.

### 5. Run cross-repo impact with `@<group>` routing

From any shell (you do **not** have to `cd` into a member repo), the normal `impact` / `query` / `context` tools accept `repo: "@<group>"` to fan out across all members, or `repo: "@<group>/<memberPath>"` to target one member. Routing is implemented in [`resolve-at-member.ts`](../../packages/core/src/core/group/resolve-at-member.ts) and described in [`tools.ts`](../../packages/core/src/mcp/tools.ts).
...
```bash
npx @codragraph/cli group impact payments-platform \
  --repo orders --target PlaceOrder --direction upstream --cross-depth 2
```

Phase 1 walks within the anchor member; Phase 2 hops across the Contract Bridge wherever a cross-link endpoint matches an impacted symbol. See [`cross-impact.ts`](../../packages/core/src/core/group/cross-impact.ts) for the bridge query.

## How gRPC extraction works

`GrpcExtractor` ([`grpc-extractor.ts`](../../packages/core/src/core/group/extractors/grpc-extractor.ts)) runs two passes per member repo:
...
2. **Source scan.** Every source file whose extension matches [`GRPC_SCAN_GLOB`](../../packages/core/src/core/group/extractors/grpc-patterns/index.ts) is parsed by its language plugin:

| Language | Provider signal | Consumer signal |
|----------|-----------------|-----------------|
| Go ([`go.ts`](../../packages/core/src/core/group/extractors/grpc-patterns/go.ts)) | `pb.RegisterXxxServer(...)`, `pb.UnimplementedXxxServer` embedded in struct | `pb.NewXxxClient(conn)` |
| Java ([`java.ts`](../../packages/core/src/core/group/extractors/grpc-patterns/java.ts)) | `extends XxxServiceGrpc.XxxServiceImplBase` (with or without `@GrpcService`) | `XxxServiceGrpc.newBlockingStub(...)`, `newStub(...)` |
| Python ([`python.ts`](../../packages/core/src/core/group/extractors/grpc-patterns/python.ts)) | `add_XxxServicer_to_server(...)` (bare or `_pb2_grpc.` attribute form) | `XxxStub(channel)` (ignores `Mock`/`Test`/`Fake`/`Stub`) |
| Node / TS ([`node.ts`](../../packages/core/src/core/group/extractors/grpc-patterns/node.ts)) | NestJS `@GrpcMethod('Service','Method')` | `@GrpcClient` field typed `XxxServiceClient`, `client.getService<X>('Service')`, `new XxxServiceClient(...)`, `new foo.bar.XxxService(...)` in files that call `loadPackageDefinition` |
...
The escape hatch is the `links` list in `group.yaml`, handled by [`ManifestExtractor`](../../packages/core/src/core/group/extractors/manifest-extractor.ts). Each entry is a **one-directional** provider/consumer declaration:
