# Using CodraGraph across gRPC microservices
...
- CodraGraph installed and runnable as `codragraph` or `npx codragraph` (see the root [README.md](../../README.md)).
...
- Write access to `~/.codragraph/` (the default codragraph home; see `getDefaultCodragraphDir` in [`storage.ts`](../../codragraph/src/core/group/storage.ts)).
...
Run `analyze` from inside each service repo (or pass the path). The CLI surface lives in [`codragraph/src/cli/analyze.ts`](../../codragraph/src/cli/analyze.ts) and is wired in [`codragraph/src/cli/index.ts`](../../codragraph/src/cli/index.ts).

```bash
cd ~/code/gateway && npx codragraph analyze
cd ~/code/orders  && npx codragraph analyze
cd ~/code/inventory && npx codragraph analyze
```
...
Each run writes a `.codragraph/` folder in the repo and registers the repo in `~/.codragraph/registry.json`. Confirm with `npx codragraph list`.

### 2. Author `group.yaml`

Create the group directory and edit the config. Either use the CLI scaffolder or write the file directly â€” both produce the same shape consumed by [`config-parser.ts`](../../codragraph/src/core/group/config-parser.ts).

```bash
npx codragraph group create payments-platform
# or manually:
...
Field notes (schema in [`types.ts`](../../codragraph/src/core/group/types.ts)):
...
- `repos` â€” a mapping from **group path** (a logical name you choose; can be a hierarchy like `backend/orders`) to **registry name** (the name shown by `npx codragraph list`). Both sides appear throughout the tooling: contract rows use the group path; `@<group>/<groupPath>` routes tools to a single member.
...
```bash
npx codragraph group sync payments-platform --verbose
```

What this does (see [`sync.ts`](../../codragraph/src/core/group/sync.ts)):

1. Opens each member's per-repo LadybugDB.
2. Runs the HTTP, gRPC, and topic extractors against the source files.
3. Applies manifest `links` through [`manifest-extractor.ts`](../../codragraph/src/core/group/extractors/manifest-extractor.ts).
...
The same operation is available over MCP as `group_sync({ name: "payments-platform" })` â€” see [`tools.ts`](../../codragraph/src/mcp/tools.ts).

### 4. Inspect the registry

Use `codragraph group contracts` for the CLI view or read the `codragraph://group/<name>/contracts` MCP resource for the same data.

```bash
npx codragraph group contracts payments-platform --type grpc --json
```
...
Staleness of the underlying indexes shows up in `npx codragraph group status payments-platform` or the `codragraph://group/<name>/status` resource.

### 5. Run cross-repo impact with `@<group>` routing

From any shell (you do **not** have to `cd` into a member repo), the normal `impact` / `query` / `context` tools accept `repo: "@<group>"` to fan out across all members, or `repo: "@<group>/<memberPath>"` to target one member. Routing is implemented in [`resolve-at-member.ts`](../../codragraph/src/core/group/resolve-at-member.ts) and described in [`tools.ts`](../../codragraph/src/mcp/tools.ts).
...
```bash
npx codragraph group impact payments-platform \
  --repo orders --target PlaceOrder --direction upstream --cross-depth 2
```

Phase 1 walks within the anchor member; Phase 2 hops across the Contract Bridge wherever a cross-link endpoint matches an impacted symbol. See [`cross-impact.ts`](../../codragraph/src/core/group/cross-impact.ts) for the bridge query.

## How gRPC extraction works

`GrpcExtractor` ([`grpc-extractor.ts`](../../codragraph/src/core/group/extractors/grpc-extractor.ts)) runs two passes per member repo:
...
2. **Source scan.** Every source file whose extension matches [`GRPC_SCAN_GLOB`](../../codragraph/src/core/group/extractors/grpc-patterns/index.ts) is parsed by its language plugin:

| Language | Provider signal | Consumer signal |
|----------|-----------------|-----------------|
| Go ([`go.ts`](../../codragraph/src/core/group/extractors/grpc-patterns/go.ts)) | `pb.RegisterXxxServer(...)`, `pb.UnimplementedXxxServer` embedded in struct | `pb.NewXxxClient(conn)` |
| Java ([`java.ts`](../../codragraph/src/core/group/extractors/grpc-patterns/java.ts)) | `extends XxxServiceGrpc.XxxServiceImplBase` (with or without `@GrpcService`) | `XxxServiceGrpc.newBlockingStub(...)`, `newStub(...)` |
| Python ([`python.ts`](../../codragraph/src/core/group/extractors/grpc-patterns/python.ts)) | `add_XxxServicer_to_server(...)` (bare or `_pb2_grpc.` attribute form) | `XxxStub(channel)` (ignores `Mock`/`Test`/`Fake`/`Stub`) |
| Node / TS ([`node.ts`](../../codragraph/src/core/group/extractors/grpc-patterns/node.ts)) | NestJS `@GrpcMethod('Service','Method')` | `@GrpcClient` field typed `XxxServiceClient`, `client.getService<X>('Service')`, `new XxxServiceClient(...)`, `new foo.bar.XxxService(...)` in files that call `loadPackageDefinition` |
...
The escape hatch is the `links` list in `group.yaml`, handled by [`ManifestExtractor`](../../codragraph/src/core/group/extractors/manifest-extractor.ts). Each entry is a **one-directional** provider/consumer declaration:
...
What the manifest extractor does (see [`manifest-extractor.ts`](../../codragraph/src/core/group/extractors/manifest-extractor.ts)):
...
History: the manifest extractor used to be silently skipped by the sync pipeline; that was fixed in PR #827 (tracking issue #826). If you ever see `config.links` with zero cross-links in `contracts.json`, make sure you're on a build that includes that fix, then re-run `group sync`.
...
- [`codragraph/src/core/group/`](../../codragraph/src/core/group/) â€” `service.ts`, `sync.ts`, `config-parser.ts`, `matching.ts`.
- [`codragraph/src/core/group/extractors/grpc-extractor.ts`](../../codragraph/src/core/group/extractors/grpc-extractor.ts) and [`grpc-patterns/`](../../codragraph/src/core/group/extractors/grpc-patterns/) â€” gRPC detection.
- [`codragraph/src/core/group/extractors/manifest-extractor.ts`](../../codragraph/src/core/group/extractors/manifest-extractor.ts) â€” the `config.links` escape hatch.
- [`codragraph/src/mcp/tools.ts`](../../codragraph/src/mcp/tools.ts) â€” MCP tool schemas (`group_list`, `group_sync`, plus `@<group>` routing on `impact` / `query` / `context`).
- [`codragraph/src/cli/group.ts`](../../codragraph/src/cli/group.ts) â€” CLI command definitions and flags.
