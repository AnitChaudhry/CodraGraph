...
<repo>/.codragraph/
  ...
  structure/     compact markdown agent memory plus SQLite seed SQL
...
`structure/` is intentionally small. It contains pre-seeded markdown files such
as `WHAT.md`, `WHY.md`, `HOW.md`, `WHEN.md`, `WHERE.md`, `BRANCHES.md`, and
`INDEX.md`, plus `state.json` and `agent-memory.sql` for tools that mirror the
state into SQLite. It is regenerated by `codragraph analyze` and smart-reuse
runs so agents can read branch/index state without opening the graph database.
