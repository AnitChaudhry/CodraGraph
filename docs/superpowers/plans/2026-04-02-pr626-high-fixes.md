...
**Architecture:** Minimal targeted fixes â€” each task is independent. TDD: tests first, then implementation. No refactoring beyond what's needed.

**Tech Stack:** TypeScript, Vitest, Node.js fs/path APIs

**Spec:** `docs/superpowers/specs/2026-04-02-pr626-high-fixes-design.md`

**Paths:** All file paths are relative to the monorepo root (`CodraGraph/`). Git commands run from the root. The `packages/core/` prefix is a package subdirectory, not a separate repo.

---

### Task 1: Path Traversal â€” Validate Group Name

**Files:**
- Modify: `packages/core/src/core/group/storage.ts:17-19` (getGroupDir) and `:63-68` (createGroupDir)
- Test: `packages/core/test/unit/group/storage.test.ts`

- [ ] **Step 1: Write failing tests for validateGroupName**

In `packages/core/test/unit/group/storage.test.ts`, add `createGroupDir` and `validateGroupName` to the existing import from `'../../../src/core/group/storage.js'` (line 6-11). Then add these describe blocks at the end of the outer `describe('Group storage', ...)`:
...
Run: `cd packages/core && npx vitest run test/unit/group/storage.test.ts`
Expected: FAIL â€” `validateGroupName` is not exported, `getGroupDir` does not throw.

- [ ] **Step 3: Implement validateGroupName and wire into getGroupDir and createGroupDir**

In `packages/core/src/core/group/storage.ts`, add the validation function before `getGroupDir` and call it:
...
```bash
cd packages/core && git add src/core/group/storage.ts test/unit/group/storage.test.ts
...
**Files:**
- Modify: `packages/core/src/core/group/service-boundary-detector.ts:24-51` (add constant), `:78` (walkForBoundaries), `:130` (hasSourceFilesInSubdirs)
- Test: `packages/core/test/unit/group/service-boundary-detector.test.ts`

- [ ] **Step 1: Write failing tests for excluded directories**

Add this describe block inside the existing `detectServiceBoundaries` describe in `packages/core/test/unit/group/service-boundary-detector.test.ts`:

```typescript
    it('test_detect_skips_vendor_directory', async () => {
      writeFile('services/auth/package.json', '{}');
      writeFile('services/auth/src/index.ts', '');
      // vendor should be skipped â€” its contents should not create a boundary
    ...
    it('test_detect_skips_pycache_directory', async () => {
      ...
      // __pycache__ with a marker + source files â€” would be detected as
...
Run: `cd packages/core && npx vitest run test/unit/group/service-boundary-detector.test.ts`
...
In `packages/core/src/core/group/service-boundary-detector.ts`:
...
```bash
cd packages/core && git add src/core/group/service-boundary-detector.ts test/unit/group/service-boundary-detector.test.ts
...
**Files:**
- Modify: `packages/core/src/cli/group.ts:160` (remove import), `:187-189` (remove finally block body)
- Test: `packages/core/test/unit/group/sync.test.ts` (add pool cleanup test)
- Test: `packages/core/test/integration/group/group-cli.test.ts` (verify no blanket close in source)

- [ ] **Step 1: Write unit tests for per-id pool cleanup in sync.ts**

Add to `packages/core/test/unit/group/sync.test.ts`, inside the existing `describe('syncGroup', ...)`:
...
```typescript
  it('test_syncGroup_closes_only_opened_pools', async () => {
    ...
    // Mock initCgdb/closeCgdb via per-repo override that tracks pool lifecycle
    ...
    try {
      ...
      // Regardless of extraction errors, closeCgdb should be called per id
      // closeCgdb should only receive specific pool ids, never undefined/empty
...
Run: `cd packages/core && npx vitest run test/unit/group/sync.test.ts`
Expected: PASS â€” sync.ts already cleans up correctly. This test locks the behavior.

- [ ] **Step 3: Write test verifying CLI source has no blanket closeCgdb()**

Add to `packages/core/test/integration/group/group-cli.test.ts`:
...
```typescript
  it('test_sync_command_source_does_not_call_blanket_closeCgdb', () => {
    ...
    // closeCgdb(id) with argument is fine (that's in sync.ts, not here).
...
Run: `cd packages/core && npx vitest run test/integration/group/group-cli.test.ts`
Expected: FAIL â€” `closeCgdb()` (no args) exists at line 188.

- [ ] **Step 5: Remove blanket closeCgdb() from cli/group.ts**

In `packages/core/src/cli/group.ts`:

Remove the `closeCgdb` import at line 160:
```typescript
      const { closeCgdb } = await import('../core/cgdb/pool-adapter.js');
```
...
Run: `cd packages/core && npx vitest run test/integration/group/group-cli.test.ts test/unit/group/sync.test.ts`
Expected: ALL PASS

- [ ] **Step 7: Commit**

```bash
cd packages/core && git add src/cli/group.ts test/integration/group/group-cli.test.ts test/unit/group/sync.test.ts
git commit -m "fix(group): remove blanket closeCgdb() from CLI sync command

sync.ts already closes pools per-id in its finally block.
The blanket closeCgdb() in cli/group.ts tears down ALL active pools
...
### Task 4: gRPC Proto Regex â€” Brace-Depth Counter

**Files:**
- Modify: `packages/core/src/core/group/extractors/grpc-extractor.ts:101-130` (parseProtoFile)
- Test: `packages/core/test/unit/group/grpc-extractor.test.ts`

- [ ] **Step 1: Write failing tests for nested braces in proto services**

Add this describe block inside the existing `proto file parsing` describe in `packages/core/test/unit/group/grpc-extractor.test.ts`:
...
service IncompleteService {
  ...
  // Missing closing brace â€” EOF before depth returns to 0
`,
      );
...
Run: `cd packages/core && npx vitest run test/unit/group/grpc-extractor.test.ts`
Expected: `test_extract_proto_with_google_api_http_nested_braces` FAIL â€” regex stops at first `}` inside the `option` block.

- [ ] **Step 3: Replace serviceRe regex with extractServiceBlocks function**

In `packages/core/src/core/group/extractors/grpc-extractor.ts`, replace the `parseProtoFile` method (lines 101-130):
...
```bash
cd packages/core && git add src/core/group/extractors/grpc-extractor.ts test/unit/group/grpc-extractor.test.ts
...
Run: `cd packages/core && npx vitest run test/unit/group/ test/integration/group/`
Expected: ALL PASS
...
Add to `packages/core/test/integration/group/group-cli.test.ts` inside the existing `group CLI` describe:
...
```bash
cd packages/core && git add test/integration/group/group-cli.test.ts
