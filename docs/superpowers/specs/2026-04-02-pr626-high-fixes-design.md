...
**File:** `codragraph/src/core/group/storage.ts`
...
- Reviewer explicitly requested `[a-zA-Z0-9_-]`
...
**File:** `codragraph/src/core/group/extractors/grpc-extractor.ts`
...
**File:** `codragraph/src/core/group/service-boundary-detector.ts`
...
**Files:**
- `codragraph/src/core/group/sync.ts` (lines 155-157) â€” per-id cleanup (KEEP)
- `codragraph/src/cli/group.ts` (line 188) â€” blanket `closeLbug()` (REMOVE)
...
Remove the `closeLbug` import from `cli/group.ts` â€” after removing the `finally` call it has no remaining usages.
...
- CLI `sync` command does not call blanket `closeLbug()` (verify no zero-arg call in source â€” static check or grep-based test)
