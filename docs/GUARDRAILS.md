...
- **Do:** `npx @codragraph/cli analyze` (plus `--embeddings` if used).
...
- **Do:** `npx @codragraph/cli analyze --embeddings`, confirm `meta.json` reflects stored embeddings.
...
- **Do:** `npx @codragraph/cli analyze` in the target repo; verify `npx @codragraph/cli list` shows it.
...
- **Trigger:** Errors opening `.codragraph/cgdb` while MCP and analyze both run.
