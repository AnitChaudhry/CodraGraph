...
## Project Overview
CodraGraph is a code intelligence tool that builds a knowledge graph from source code using tree-sitter AST parsing across 12 languages and KuzuDB for graph storage. Two packages: `codragraph/` (CLI/MCP, TypeScript) and `codragraph-web/` (browser).

## Cross-Language Pattern Consistency (pattern-recognition-specialist)
- 12 language-specific type extractors in `codragraph/src/core/ingestion/type-extractors/` must follow identical patterns for: async unwrapping, constructor binding, namespace handling, nullable type stripping, for-loop element typing.
...
- KuzuDB graph operations: schema in `codragraph/src/core/kuzu/schema.ts`, adapter in `kuzu-adapter.ts`.
...
- `cgdb-adapter.ts` fallback path needs quote/newline escaping for Cypher injection prevention.

## Security (security-sentinel)
- Cypher query construction in `cgdb-adapter.ts` and `kuzu-adapter.ts` — watch for injection via unescaped user-provided symbol names.
...
- `codragraph-web` package drifts behind CLI — flag if a change should be mirrored.
...
**Why:** CodraGraph is an MCP server — all tool inputs come from untrusted AI agents. Systematic OWASP-level audit catches injection vectors that spot-checking misses. Past finding: `cgdb-adapter.ts` fallback path had unescaped newlines in Cypher queries.

### voltagent-data-ai:database-optimizer
**When:** Changes touch `kuzu-adapter.ts`, `schema.ts`, `cgdb-adapter.ts`, or any Cypher query construction/execution.
...
## Review Tooling
- Use `codragraph_impact()` before approving changes to any symbol — check d=1 (WILL BREAK) callers.
- Use `codragraph_detect_changes({scope: "compare", base_ref: "main"})` to map PR diffs to affected execution flows.
