...
- `codragraph-harness-tuned`, `codragraph-swarm-tuned`, `codragraph-recipe-cached` treatments throw a clear error instructing you to run `swarm-tune.ts` first and then load the recipe via the candidate filesystem. The next iteration of `treatments.ts` will accept a `--recipe-dir` flag and use `compileAndLoadCandidate` to load the persisted recipe automatically.
