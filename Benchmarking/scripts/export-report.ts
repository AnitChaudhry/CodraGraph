...
function renderMarkdown(s: SummaryFile): string {
  ...
  lines.push("|---|---|");
  lines.push(`| Workload | \`${s.env.workload.id}\` v${s.env.workload.version} (${s.env.workload.taskCount} tasks; split: ${s.env.workload.split}) |`);
  lines.push(`| Seeds | ${s.env.seeds.join(", ")} |`);
  lines.push(`| Judge | ${s.env.judge.provider} / ${s.env.judge.model} |`);
  lines.push(`| Codragraph | ${s.env.codragraph.version}; indexed repo SHA: ${s.env.codragraph.indexedRepoSha} |`);
  lines.push(`| Host | ${s.env.host.os}; ${s.env.host.cpu}; ${s.env.host.ramGb} GB RAM |`);
  if (s.env.host.gpus.length > 0) {
    lines.push(`| GPU | ${s.env.host.gpus.map((g) => `${g.name} (${g.memoryGb} GB)`).join(", ")} |`);
  }
  lines.push(`| Node | ${s.env.node.version} on ${s.env.node.platform} |`);
  if (s.env.git.dirty) {
    lines.push(`| **⚠️ Git** | dirty working tree at ${s.env.git.sha} (results not strictly reproducible) |`);
  } else {
  ...
  lines.push("");
  lines.push("| Treatment | Model | Accuracy (mean ± std) | 95% CI | Mean tokens | p95 tokens | Mean latency | Cost |");
  lines.push("|---|---|---|---|---|---|---|---|");
  ...
  for (const [modelId, cells] of byModel) {
    ...
    for (const cell of cells) {
      ...
      const tokenRed = baseline.aggregate.tokensMeanMean > 0
  ...
  if (headline) {
    ...
    lines.push(`(No headline generated — need at least one model with both \`baseline-grep\` and a \`codragraph-*\` treatment.)`);
  }
  lines.push("");
  ...
  lines.push("");
  lines.push("- Each cell ran **${s.env.seeds.length} seeds** with mean-of-means aggregation.".replace("${s.env.seeds.length}", String(s.env.seeds.length)));
...
function generateHeadline(s: SummaryFile): string | null {
  ...
  let best: { modelId: string; treatment: TreatmentTag; accDelta: number; tokenRed: number } | null = null;
  for (const [modelId, cells] of byModel) {
    const baseline = cells.find((c) => c.treatment === "baseline-grep");
    if (!baseline) continue;
    for (const cell of cells) {
