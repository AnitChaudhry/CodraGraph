...
export const QueryFAB = () => {
  ...
  if (!isExpanded) {
    return (
      <button
        ...
        className="group absolute bottom-4 left-4 z-20 flex items-center gap-2 rounded-lg border border-cyan-500/35 bg-elevated/95 px-4 py-2.5 text-sm font-medium text-cyan-100 shadow-lg shadow-black/25 backdrop-blur transition-colors duration-200 hover:border-cyan-400/60 hover:bg-hover"
      >
        <Terminal className="h-4 w-4 text-cyan-300" />
        <span>Query</span>
        {queryResult && queryResult.nodeIds.length > 0 && (
          <span className="ml-1 rounded-md border border-cyan-400/20 bg-cyan-500/15 px-1.5 py-0.5 text-xs font-semibold text-cyan-100">
  ...
  return (
    <div
      ref={panelRef}
      className="absolute bottom-4 left-4 z-20 w-[480px] max-w-[calc(100%-2rem)] animate-fade-in rounded-lg border border-border-default bg-deep/95 shadow-2xl shadow-black/30 backdrop-blur-md"
    >
      <div className="flex items-center justify-between border-b border-border-subtle px-4 py-3">
        <div className="flex items-center gap-2">
          <div className="flex h-7 w-7 items-center justify-center rounded-md border border-cyan-500/25 bg-cyan-500/10">
            <Terminal className="h-4 w-4 text-cyan-300" />
          </div>
        ...
        <div className="mt-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-md bg-cyan-500 px-4 py-1.5 text-sm font-medium text-slate-950 transition-colors hover:bg-cyan-400 disabled:cursor-not-allowed disabled:opacity-50 disabled:shadow-none"
            >
              {isRunning ? (
              ...
              <span>Run</span>
              <kbd className="ml-1 rounded bg-slate-900/20 px-1 py-0.5 text-[10px]">Ctrl Enter</kbd>
            </button>
          </div>
        </div>
      </div>
      ...
      {queryResult && !error && (
        <div className="border-t border-cyan-500/20">
          ...
          {showResults && queryResult.rows.length > 0 && (
            <div className="scrollbar-canvas max-h-48 overflow-auto border-t border-border-subtle">
