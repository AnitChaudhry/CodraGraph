...
2. **CLI / MCP package:** `cd codragraph && npm install && npm run build`
3. **Web UI (if needed):** `cd codragraph-web && npm install`
...
- [ ] Tests pass for the packages you touched (`codragraph` and/or `codragraph-web`).
- [ ] Typecheck passes: `npx tsc --noEmit` in `codragraph/` and `npx tsc -b --noEmit` in `codragraph-web/`.
