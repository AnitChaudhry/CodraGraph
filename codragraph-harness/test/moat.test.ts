...
describe("findReusableRecipes", () => {
  it("returns exact matches and ranked candidates", async () => {
    ...
    await store.put(sampleInput({ snapshotId: exactSnap, paretoCoords: { accuracy: 0.9, tokens: 1000, latencyMs: 500 }, scores: { accuracy: 0.9, tokens: 1000, latencyMs: 500, taskCount: 30 } }));
    await store.put(
      sampleInput({
