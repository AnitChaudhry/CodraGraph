...
program
  ...
  .requiredOption(
    '-t, --task <path>',
    'JSON file with the task set (see test/fixtures/qa-test-set.json)',
  )
...
program
  ...
  .option('--plateau-k <k>', "Stop when frontier hasn't changed for K iterations (omit to disable)")
  .option('--token-budget <n>', 'Stop when total proposer+eval tokens exceed N (omit to disable)')
  .option('--time-budget-ms <ms>', 'Stop when wall-clock exceeds ms (omit to disable)')
  .option('--cost-budget-usd <usd>', 'Stop when estimated cost exceeds USD (omit to disable)')
...
recipesCmd
  ...
  .action(async (opts: { keepN: string; store: string; dryRun?: boolean }) => {
...
recipesCmd
  .command('lookup')
  .description(
    'Find recipes for a (snapshot-id, task-family) pair, with staleness if --differ-mock',
  )
  ...
  const seeds: Harness[] =
    opts.seeds === 'all'
      ...
      : opts.seeds.split(',').map((name) => {
          ...
          if (!seed)
            throw new Error(
              `Unknown seed: ${name}. Available: ${Object.keys(SEEDS_BY_NAME).join(', ')}`,
            );
          return seed;
        });
...
function defaultOnProgress(event: ProgressEvent): void {
  switch (event.type) {
    ...
    case 'seed-evaluated':
      console.error(
        `[seed] ${event.id} acc=${event.scores.accuracy.toFixed(3)} tokens=${event.scores.tokens.toFixed(0)}`,
      );
      break;
    ...
    case 'candidate-evaluated':
      console.error(
        `  [eval] ${event.id} acc=${event.scores.accuracy.toFixed(3)} tokens=${event.scores.tokens.toFixed(0)}`,
      );
      break;
    case 'complete':
...
async function runSwarmSearch(opts: SwarmCliOptions): Promise<void> {
  ...
  if (opts.useCache && !moatEnabled) {
    throw new Error('--use-cache requires both --task-family <name> and --snapshot-id <id>');
  }
  ...
  console.error(`Run dir: ${runDir}`);
  console.error(
    `Terminated: ${result.terminatedAt.reason} at iteration ${result.terminatedAt.iteration}`,
  );
  console.error(`Total evaluated: ${result.totalEvaluated}`);
