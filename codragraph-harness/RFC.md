# RFC — codragraph-harness Phase 1
...
4. **Three distribution surfaces**: CLI (`codragraph-harness`), MCP tool
   (`harness_run` registered in `codragraph/src/mcp/tools.ts`), and npm SDK
   re-export (`codragraph-sdk`).
...
```
codragraph-harness/
├── package.json                         npm: "codragraph-harness", v0.1.0
├── tsconfig.json                        ESM, strict, target ES2022
├── vitest.config.ts                     test runner config
├── README.md                            user-facing intro
├── RFC.md                               this doc
├── src/
│   ├── index.ts                         (placeholder; SDK consumes via codragraph-sdk)
...
│   └── cli/
│       └── main.ts                      bin/codragraph-harness
└── test/
...
The proposer handler is **out-of-process**: codragraph-harness exposes
`handleHarnessRun()` from `src/mcp/handler.ts`, and codragraph's MCP
server (in `codragraph/src/mcp/local/` per their dispatch pattern) is
expected to import and route the `harness_run` tool to it. This avoids
making codragraph-harness a hard runtime dep of codragraph.

**TODO:** Add the actual handler dispatch wiring in
`codragraph/src/mcp/local/`. Currently `tools.ts` declares the tool but
...
Also: `codragraph-harness list-runs`, `codragraph-harness show <runDir>`.
...
`codragraph/src/mcp/tools.ts`. Inputs:
...
Implementation: `codragraph-harness/src/mcp/handler.ts:handleHarnessRun`.

## 11. SDK surface (codragraph-sdk)
...
Or sub-namespaced: `codragraph-sdk/harness`, `codragraph-sdk/graph`.

Compression namespace lands in Phase 1.5. Today the SDK is a thin
re-export of codragraph-harness's public surface plus the HTTP graph
client.
...
1. **Wire `harness_run` handler dispatch** in `codragraph/src/mcp/local/` (the
...
| 1.5 | Port `codragraph-compress` to TS | Pending |
