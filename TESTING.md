...
| -------------- | -------------- | -------- | ------------------------------ |
| CLI + MCP core | `codragraph/`    | Vitest   | Primary test surface in CI     |
| Web UI         | `codragraph-web/`| Vitest   | Unit/component tests           |
| Web UI E2E     | `codragraph-web/`| Playwright | Run when changing UI flows   |
...
2. **`codragraph-web/` files staged** → `tsc -b --noEmit`
3. **`codragraph/` files staged** → `tsc --noEmit`
...
- **Integration** — Real combinations (filesystem, MCP wiring, larger pipelines) as already organized under `codragraph/test/integration`.
...
- **`ci-quality.yml`** — prettier format check, eslint lint, `tsc --noEmit` for `codragraph/`, `tsc -b --noEmit` for `codragraph-web/`
- **`ci-tests.yml`** — `vitest run` with coverage (ubuntu) + cross-platform (macOS, Windows)
- **`ci-e2e.yml`** — Playwright E2E tests, gated on `codragraph-web/**` changes

Local checks before pushing:

```bash
cd codragraph && npx tsc --noEmit && npm test
cd ../codragraph-web && npx tsc -b --noEmit && npm test
```
