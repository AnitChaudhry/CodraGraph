...
describe('loadSettings', () => {
  ...
  it('migrates legacy localStorage to sessionStorage', () => {
    ...
    expect(sessionStorage.getItem('codragraph-llm-settings')).not.toBeNull();
    expect(localStorage.getItem('codragraph-llm-settings')).toBeNull();
  });
});
...
describe('saveSettings / clearSettings', () => {
  ...
  it('clearSettings removes settings from both storages', () => {
    ...
    expect(sessionStorage.getItem('codragraph-llm-settings')).not.toBeNull();
    clearSettings();
    expect(sessionStorage.getItem('codragraph-llm-settings')).toBeNull();
    expect(localStorage.getItem('codragraph-llm-settings')).toBeNull();
  });
});
