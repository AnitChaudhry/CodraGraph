...
| codragraph-org depends on codragraph-graphstore + codragraph-shared | Avoids the circular import; the engine packages stay independent of the org layer. |
...
4. **codragraph server integration** — add a feature flag
   ...
   `codragraph/src/server/api.ts` runs inside `withTenant` derived
...
| 2   | Dashboard MVP in codragraph-web              | Shipped |
