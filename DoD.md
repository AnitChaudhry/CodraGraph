...
- CLI, MCP, and HTTP-bridge behavior in `codragraph/`
- Browser UI in `codragraph-web/`
- Shared contracts in `codragraph-shared/`
...
- [ ] The change is placed in the correct package and layer:
  - `codragraph/` for CLI, MCP, HTTP bridge, ingestion, graph, and runtime logic
  - `codragraph-web/` for browser UI (thin client — no WASM workers, all queries via HTTP API)
  - `codragraph-shared/` for shared contracts, types, and constants
- [ ] Pipeline and architecture boundaries remain explicit. Shared ingestion code in `codragraph/src/core/ingestion/` must not name languages — use `LanguageProvider` hooks (see `AGENTS.md` and `ARCHITECTURE.md` § Call-Resolution DAG).
...
- [ ] Direct imports from `codragraph-shared` are used. No barrel re-exports introduced to paper over drift between packages.
...
- [ ] Existing contracts (types in `codragraph-shared/`, CLI flags, MCP tools/resources, HTTP routes, graph node/edge shapes, persisted IDs) are preserved unless the task explicitly requires a contract change.
...
- [ ] `codragraph-shared/` dist is built before consuming packages are typechecked or tested (CI uses the `setup-codragraph` action for this — local runs must match).

### 4.2 If `codragraph/` changed

- [ ] `cd codragraph && npx tsc --noEmit`
- [ ] `cd codragraph && npm test`
- [ ] `cd codragraph && npx prettier --check .` for files in the diff (pre-commit runs the affected-tests subset; do not expand scope)

### 4.3 If `codragraph-web/` changed

- [ ] `cd codragraph-web && npx tsc -b --noEmit`
- [ ] `cd codragraph-web && npm test`
- [ ] `cd codragraph-web && npm run test:e2e` when browser flows or user-facing UI behavior changed

### 4.4 If `codragraph-shared/` changed

- [ ] Shared package builds cleanly (`npm run build` in `codragraph-shared/`)
...
- A contract drifted between `codragraph/`, `codragraph-web/`, and `codragraph-shared/` and only one side was updated.
