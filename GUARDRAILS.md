...
2. **Never rename with find-and-replace** in CodraGraph-indexed projects — use `rename` MCP tool with `dry_run: true` first, review `graph` vs `text_search` edits. No separate `codragraph rename` CLI exists.
...
5. **Preserve embeddings** — if `.codragraph/meta.json` shows embeddings, use `npx codragraph analyze --embeddings`; plain `analyze` drops them.
...
- **Do:** `npx codragraph analyze` (plus `--embeddings` if used).
...
- **Do:** `npx codragraph analyze --embeddings`, confirm `meta.json` reflects stored embeddings.
...
- **Do:** `npx codragraph analyze` in the target repo; verify `npx codragraph list` shows it.
- **Why:** MCP discovers repos via `~/.codragraph/registry.json`, populated by analyze.
...
- **Trigger:** Errors opening `.codragraph/lbug` while MCP and analyze both run.
