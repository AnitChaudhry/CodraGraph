...
- **Configurable large-file skip threshold** — the walker's 512 KB default is now overridable via `CODRAGRAPH_MAX_FILE_SIZE` (KB) or `codragraph analyze --max-file-size <kb>`. Values are clamped to the 32 MB tree-sitter ceiling, invalid inputs fall back to the default with a one-time warning, and the CLI banner reports the effective post-clamp threshold when an override is active (#991, #1044).
...
- codragraph-web dependency bumps: `mermaid` 11.12.2 → 11.14.0 (#860), `tailwindcss` (#861), `jsdom` 29.0.0 → 29.0.2 (#863), `wait-on` 8.0.5 → 9.0.5 (#859), `@vitest/coverage-v8` (#864)
...
- **Runtime dependency on `file:../codragraph-shared`** removed from the published package (#803)
...
### Fixed
- **`codragraph-shared` module not found** — `codragraph-shared` was a `file:` workspace dependency never published to npm, causing `ERR_MODULE_NOT_FOUND` when installing `codragraph` globally. The build now bundles shared code into `dist/_shared/` and rewrites imports to relative paths (#613)
...
### Fixed
- Incomplete fix for `codragraph-shared` bundling — `prepare` script overwrote rewritten imports during publish
...
- **Unified web & CLI ingestion pipeline** — complete architectural migration of the web app from a self-contained WASM browser app to a thin client backed by the CLI server; new `codragraph-shared` package for cross-package type unification (#536)
...
### Docs
- Added `codragraph-shared` build step to web UI quick start instructions (#585)
