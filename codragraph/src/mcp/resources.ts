 ...
 * All resources use repo-scoped URIs: codragraph://repo/{name}/context
 */
...
export function getResourceDefinitions(): ResourceDefinition[] {
  return [
    ...
    {
      uri: 'codragraph://setup',
      name: 'CodraGraph Setup Content',
...
export function getResourceTemplates(): ResourceTemplate[] {
  return [
    {
      uriTemplate: 'codragraph://repo/{name}/context',
      name: 'Repo Overview',
      description: 'Codebase stats, staleness check, and available tools',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'codragraph://repo/{name}/clusters',
      name: 'Repo Modules',
    ...
    {
      uriTemplate: 'codragraph://repo/{name}/schema',
      name: 'Graph Schema',
    ...
    {
      uriTemplate: 'codragraph://repo/{name}/graphstore/log',
      name: 'Graph Commit Log',
      description:
        'Phase 4: history of content-addressed snapshots for the indexed repo, newest-first. Pairs with the graphstore_log tool.',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'codragraph://repo/{name}/graphstore/branches',
      name: 'Graph Branches',
      description:
        'Phase 4: list of versioned-graph branches with their head commits. Pairs with graphstore_branches.',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'codragraph://repo/{name}/graphstore/head',
      name: 'Graph HEAD',
      description:
        "Phase 4: the indexed repo's current branch + head commit, plus the snapshot it points at. Pairs with graphstore_log/diff/blame.",
      mimeType: 'text/yaml',
    },
    {
    ...
    {
      uriTemplate: 'codragraph://repo/{name}/recipes/{family}',
      name: 'Recipes for Task Family',
      description:
        'Phase 4 × Phase 3 moat: recipes filtered to a specific task family across all snapshots — visualizes how the recipe set evolves as the codebase changes.',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'codragraph://group/{name}/contracts',
    ...
    {
      uriTemplate: 'codragraph://group/{name}/status',
...
/** Query parameters for `codragraph://group/{name}/contracts` */
...
/** Normalized parse result for CodraGraph MCP resource URIs */
export type ParsedCodragraphResource =
  | { kind: 'repos' }
  | { kind: 'setup' }
  | {
      kind: 'repo';
...
/**
 * Parse a CodraGraph resource URI (repos, setup, per-repo, or per-group templates).
 * Used by `readResource` and tests (round-trip / dispatch coverage).
 */
export function parseResourceUri(uri: string): ParsedCodragraphResource {
  if (uri === 'codragraph://repos') return { kind: 'repos' };
  if (uri === 'codragraph://setup') return { kind: 'setup' };
  ...
  if (u.hostname === 'group') {
    ...
    if (segments.length < 2) {
      throw new Error(
        `Invalid group resource URI (expected codragraph://group/{name}/contracts or .../status): ${uri}`,
      );
    }
...
export async function readResource(uri: string, backend: LocalBackend): Promise<string> {
  ...
  switch (parsed.resourceType) {
    ...
    case 'graphstore/log':
      return getGraphstoreLogResource(backend, repoName);
    case 'graphstore/branches':
    ...
    default:
      if (parsed.resourceType.startsWith('recipes/')) {
        const family = parsed.resourceType.slice('recipes/'.length);
        return getRecipesResource(backend, repoName, family);
      }
...
async function getReposResource(backend: LocalBackend): Promise<string> {
  ...
  if (repos.length === 0) {
    return 'repos: []\n# No repositories indexed. Run: codragraph analyze';
  }
  ...
  if (repos.length > 1) {
    ...
    lines.push(`# codragraph_search({query: "auth", repo: "${repos[0].name}"})`);
  }
...
async function getContextResource(backend: LocalBackend, repoName?: string): Promise<string> {
  ...
  if (!context) {
    return 'error: No codebase loaded. Run: codragraph analyze';
  }
...
async function getClustersResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.clusters || result.clusters.length === 0) {
      return 'modules: []\n# No functional areas detected. Run: codragraph analyze';
    }
    ...
    if (result.clusters.length > displayLimit) {
      lines.push(
        `\n# Showing top ${displayLimit} of ${result.clusters.length} modules. Use codragraph_query for deeper search.`,
      );
    }
...
async function getProcessesResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.processes || result.processes.length === 0) {
      return 'processes: []\n# No processes detected. Run: codragraph analyze';
    }
    ...
    if (result.processes.length > displayLimit) {
      lines.push(
        `\n# Showing top ${displayLimit} of ${result.processes.length} processes. Use codragraph_query for deeper search.`,
      );
    }
...
function getSchemaResource(): string {
  return `# CodraGraph Graph Schema
 ...
 * Useful for `codragraph setup` onboarding or dynamic content injection.
 */
async function getSetupResource(backend: LocalBackend): Promise<string> {
  ...
  for (const repo of repos) {
    ...
    const lines = [
      `# CodraGraph MCP — ${repo.name}`,
      '',
      `This project is indexed by CodraGraph as **${repo.name}** (${stats.nodes || 0} symbols, ${stats.edges || 0} relationships, ${stats.processes || 0} execution flows).`,
      '',
      '## Tools',
      '',
...
async function getGraphstoreLogResource(backend: LocalBackend, repoName?: string): Promise<string> {
  ...
  let result;
  try {
    result = await handler.handleGraphstoreLog({
  ...
  if (result.commits.length === 0) {
    return [
      `repo: "${repo.name}"`,
      'commits: []',
      '# No graph commits yet. Run `codragraph analyze` to create the first snapshot.',
    ].join('\n');
  }
  const lines: string[] = [`repo: "${repo.name}"`, `branch: "${result.branch ?? '(detached)'}"`];
  lines.push('commits:');
  for (const c of result.commits) {
    lines.push(`  - id: "${c.id}"`);
    lines.push(`    short: "${c.short}"`);
    lines.push(`    snapshot: "${c.snapshot}"`);
    lines.push(`    ts: "${c.ts}"`);
    lines.push(`    author: "${c.author}"`);
    lines.push(`    message: ${JSON.stringify(c.message)}`);
    if (c.parents.length > 0) {
      lines.push(`    parents: [${c.parents.map((p) => `"${p}"`).join(', ')}]`);
    }
  }
  return lines.join('\n');
}

async function getGraphstoreBranchesResource(
  backend: LocalBackend,
  repoName?: string,
): Promise<string> {
  const repo = await backend.resolveRepo(repoName);
  ...
  let result;
  try {
    result = await handler.handleGraphstoreBranches({ storagePath: repo.storagePath });
  } catch (err) {
    return `error: ${err instanceof Error ? err.message : String(err)}`;
  }
  ...
  for (const b of result.branches) {
    ...
    lines.push(`    short: "${b.short}"`);
    lines.push(`    createdAt: "${b.createdAt}"`);
    lines.push(`    isCurrent: ${b.isCurrent}`);
  }
  return lines.join('\n');
}
...
async function getRecipesResource(
  ...
  try {
    ...
    result = await mod.handleHarnessRecipesList({
      ...
      recipe_store: `${repo.storagePath}/recipes`.replace(/\\/g, '/'),
      limit: 100,
    });
  } catch (err) {
    return `error: ${err instanceof Error ? err.message : String(err)}`;
  ...
  if (result.recipes.length === 0) {
    return [
      ...
      taskFamily ? `taskFamily: "${taskFamily}"` : '',
      'recipes: []',
      '# No recipes yet. Run harness_swarm_run with task_family + snapshot_id to learn the first one.',
    ]
      .filter(Boolean)
      .join('\n');
  }
  ...
  for (const r of result.recipes) {
    ...
    lines.push(`    harness: "${r.harnessName}"`);
    lines.push(`    searchedAt: "${r.searchedAt}"`);
  }
  return lines.join('\n');
}

async function getGraphstoreHeadResource(
  backend: LocalBackend,
  repoName?: string,
): Promise<string> {
  const repo = await backend.resolveRepo(repoName);
  ...
  let log, branches;
  try {
    log = await handler.handleGraphstoreLog({ storagePath: repo.storagePath, limit: 1 });
    branches = await handler.handleGraphstoreBranches({ storagePath: repo.storagePath });
  } catch (err) {
    return `error: ${err instanceof Error ? err.message : String(err)}`;
  }
  if (log.commits.length === 0) {
    return [`repo: "${repo.name}"`, 'head: null', '# No commits yet.'].join('\n');
  }
  const head = log.commits[0]!;
  const lines: string[] = [
    `repo: "${repo.name}"`,
    `branch: "${branches.current ?? '(detached)'}"`,
    'head:',
    `  id: "${head.id}"`,
    `  short: "${head.short}"`,
    ...
    `  ts: "${head.ts}"`,
    `  author: "${head.author}"`,
    `  message: ${JSON.stringify(head.message)}`,
    '',
    '# Pair with: graphstore_log, graphstore_diff, graphstore_blame_symbol',
  ];
  return lines.join('\n');
}
