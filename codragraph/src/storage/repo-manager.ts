 ...
 * Repository Manager
 *
 * Manages CodraGraph index storage in .codragraph/ at repo root.
 * Also maintains a global registry at ~/.codragraph/registry.json
 ...
 * passed `codragraph remove some-alias` and the alias misses every
...
export interface RepoMeta {
  ...
  remoteUrl?: string;
  /**
   * Phase 4 — versioned graph storage. `currentBranch` is the
   * graphstore branch the most recent analyze advanced; `headCommit`
   * is the commit id that branch now points at. Both `undefined` when
   * the graphstore hook didn't produce a snapshot (best-effort path —
   * see `core/graphstore/`). Read by the dashboard and the upcoming
   * `codragraph log` / `diff` CLI commands; not consulted by the live
   * query path which still goes straight to LadybugDB.
   */
...
/**
 * Shape of an entry in the global registry (~/.codragraph/registry.json)
 */
export interface RegistryEntry {
  ...
  remoteUrl?: string;
  /** Mirrored from {@link RepoMeta.currentBranch} when available. */
  currentBranch?: string;
  /** Mirrored from {@link RepoMeta.headCommit} when available. */
  headCommit?: string;
...
/**
 * Get the .codragraph storage path for a repository
 */
export const getStoragePath = (repoPath: string): string => {
  return path.join(path.resolve(repoPath), CODRAGRAPH_DIR);
};
...
/**
 * Check if a path has a CodraGraph index
 */
...
/**
 * Find .codragraph by walking up from a starting path
 */
...
/**
 * Add .codragraph to .gitignore if not already present
 */
export const addToGitignore = async (repoPath: string): Promise<void> => {
  ...
  try {
    ...
    if (content.includes(CODRAGRAPH_DIR)) return;

    const newContent = content.endsWith('\n')
      ? `${content}${CODRAGRAPH_DIR}\n`
      : `${content}\n${CODRAGRAPH_DIR}\n`;
    await fs.writeFile(gitignorePath, newContent, 'utf-8');
  } catch {
    // .gitignore doesn't exist, create it
    await fs.writeFile(gitignorePath, `${CODRAGRAPH_DIR}\n`, 'utf-8');
  }
};

// ─── Global Registry (~/.codragraph/registry.json) ───────────────────────

/**
 * Get the path to the global CodraGraph directory
 */
export const getGlobalDir = (): string => {
  return process.env.CODRAGRAPH_HOME || path.join(os.homedir(), '.codragraph');
};
 ...
 * Called after `codragraph analyze` completes.
 *
 ...
 * `storagePath` does NOT point at the expected `<entry.path>/.codragraph`
 ...
 * cause is a corrupted or hand-edited `~/.codragraph/registry.json`, and
  ...
  constructor(
    ...
    super(
      `Refusing to remove storage path for safety: expected ` +
        `"${expectedStoragePath}" under the repo's .codragraph subfolder, ` +
        `but the registry entry has "${actualStoragePath}". ` +
        `This usually means the registry entry is corrupted or was ` +
        `hand-edited. Delete the entry manually from ~/.codragraph/registry.json ` +
        `and re-run analyze.`,
    );
 ...
 * registry entry's `storagePath` is the canonical `<repo>/.codragraph`
 ...
 *   - `~/.codragraph/registry.json` is a plain-text user-writable file.
...
export const assertSafeStoragePath = (entry: RegistryEntry): void => {
  const expected = path.join(path.resolve(entry.path), '.codragraph');
...
/**
 * Resolve a user-supplied target string (from `codragraph remove <target>`
 ...
 * Optionally validates that each entry's .codragraph/ still exists.
 */
export const listRegisteredRepos = async (opts?: {
  ...
  // Validate each entry still has a .codragraph/ directory
...
// ─── Global CLI Config (~/.codragraph/config.json) ─────────────────────────
//
// Single canonical config file shared by:
//   - the codragraph CLI (wiki, semantic ops)
//   - the codragraph-harness inference providers
//   - the web settings panel
//
// Backwards compat: the flat `apiKey` / `provider` / etc. fields predate
// the multi-provider model and are read by the wiki command. Newer
// callers should use the `providers` map; helpers below smooth the
// transition by promoting flat fields into the matching provider slot
// when only the legacy shape is present.

/**
 * Identifiers for inference / API-key providers known to CodraGraph.
 * Adding a new one is a one-line union extension; tooling that reads
 * the config switches on this name.
 */
...
export interface ProviderConfig {
  /** API key (BYO). When absent, fallback is the matching env var. */
  apiKey?: string;
  /** Optional override of the provider's default base URL. */
  baseUrl?: string;
  /** Default model for this provider when one isn't specified per-call. */
  model?: string;
  /** Azure-only: api-version query param (e.g. '2024-10-21'). */
  apiVersion?: string;
  /** Set true when the deployment is a reasoning model (Azure only — auto-detected for OpenAI). */
  isReasoningModel?: boolean;
  /** Free-form metadata users can attach (cursorModel, deployment notes, etc.). */
  meta?: Record<string, unknown>;
}

export interface CLIConfig {
  // ── Legacy flat fields (kept for backwards compat with wiki cmd) ──
  apiKey?: string;
  model?: string;
  baseUrl?: string;
  ...
  // ── Multi-provider keys (new canonical surface) ──────────────────
  /**
   * Per-provider config keyed by {@link ProviderName}. Read by the
   * harness's makeInferenceProvider, the web settings panel, and
   * future MCP tools that need credentials.
   */
  providers?: Partial<Record<ProviderName, ProviderConfig>>;

  /**
   * Reserved for the future hosted/org plane. When set, codragraph
   * tools that talk to a hosted CodraGraph instance authenticate with
   * this key. v0.1 ships the slot only — no server validates it yet.
   */
  codragraphApiKey?: string;
  /** Hosted org instance URL, when applicable. */
  codragraphServerUrl?: string;
}
...
/**
 * Load CLI config from ~/.codragraph/config.json
 */
...
/**
 * Save CLI config to ~/.codragraph/config.json
 */
...
// ─── Multi-provider config helpers ─────────────────────────────────────
//
// These wrap the file-based config so callers can `getProviderConfig`
// without thinking about the legacy flat fields vs the new `providers`
// map. When the multi-provider entry is present it wins; otherwise we
// promote the matching legacy field so older configs keep working.

/** Default env var per provider — used as the lowest-priority fallback. */
const PROVIDER_ENV_VARS: Record<ProviderName, string> = {
  ...
  custom: 'CODRAGRAPH_CUSTOM_API_KEY',
};

/**
 * Resolve a provider's effective config. Resolution order:
 *   1. config.providers[name]              (canonical, multi-provider)
 *   2. config.{apiKey,model,...} when the legacy provider field matches
...
export const getProviderConfig = (
  ...
  // Legacy promotion: only when the flat `provider` field matches AND
  // an apiKey is present.
  if (config.provider === name && config.apiKey) {
    const promoted: ProviderConfig = { apiKey: config.apiKey };
...
/**
 * Update a single provider's config. Persists to disk. Pass `patch`
 * with only the fields you want to change — others are merged.
 */
export const setProviderConfig = async (
  ...
  await saveCLIConfig({ ...config, providers });
};

/** Delete a provider's config. Idempotent. */
export const removeProviderConfig = async (name: ProviderName): Promise<void> => {
  const config = await loadCLIConfig();
  if (!config.providers || !(name in config.providers)) return;
  const providers = { ...config.providers };
  delete providers[name];
  await saveCLIConfig({ ...config, providers });
};

/** List every configured provider with non-secret fields. Used by `codragraph config list`. */
export const listProviderConfigs = async (): Promise<
  Array<{ name: ProviderName; hasKey: boolean; baseUrl?: string; model?: string }>
> => {
  const config = await loadCLIConfig();
  const out: Array<{
    name: ProviderName;
  ...
  for (const name of Object.keys(PROVIDER_ENV_VARS) as ProviderName[]) {
