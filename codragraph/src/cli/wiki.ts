 ...
 * Usage: codragraph wiki [path] [options]
 */
...
export const wikiCommand = async (inputPath?: string, options?: WikiCommandOptions) => {
  ...
  if (!meta) {
    ...
    console.log('  Run `codragraph analyze` first to index this repository.\n');
    process.exitCode = 1;
    return;
  }
  ...
  if (
    ...
    console.log('  Config saved to ~/.codragraph/config.json\n');
  }
  ...
  if (!hasSavedConfig && !hasCLIOverrides) {
    if (!process.stdin.isTTY) {
      ...
      if (!llmConfig.apiKey && llmConfig.provider !== 'cursor') {
        ...
        console.log('  Set OPENAI_API_KEY or CODRAGRAPH_API_KEY environment variable,');
      ...
      if (choice === '5' && hasCursor) {
        ...
        const envKey = process.env.CODRAGRAPH_API_KEY || process.env.OPENAI_API_KEY || '';
        let azureKey: string;
        if (envKey) {
  ...
  try {
    ...
    if (options?.review && result.moduleTree) {
      ...
      if (choice === 'n' || choice === 'no') {
        console.log('\n  Generation cancelled. Run `codragraph wiki` later to generate.\n');
        return;
      }
      ...
      if (choice === 'edit' || choice === 'e') {
        ...
        try {
          ...
          console.log('  Then run `codragraph wiki` to continue.\n');
          return;
        }
      }
    ...
    if (err.message?.includes('No source files')) {
      ...
      if (isAuthError && process.stdin.isTTY) {
        ...
        if (!answer || answer === 'y' || answer === 'yes') {
          ...
          await saveCLIConfig({});
          console.log('  Config cleared. Run `codragraph wiki` again to reconfigure.\n');
        }
      }
    } else {
      console.log(`\n  Error: ${err.message}\n`);
      if (process.env.CODRAGRAPH_VERBOSE) {
        console.error(err);
      }
    }
    process.exitCode = 1;
...
function publishGist(htmlPath: string): { url: string; rawUrl: string } | null {
  try {
    const output = execFileSync(
      'gh',
      ['gist', 'create', htmlPath, '--desc', 'Repository Wiki — generated by CodraGraph', '--public'],
