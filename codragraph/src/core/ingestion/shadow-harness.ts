 ...
 * ## Activation
 *
 * `CODRAGRAPH_SHADOW_MODE=1` (or `'true'`, `'yes'`, case-insensitive,
...
export interface ShadowHarness {
  /** `true` iff `CODRAGRAPH_SHADOW_MODE` is truthy. When `false`, `record()` is a no-op. */
...
/**
 * Construct a harness. Reads `CODRAGRAPH_SHADOW_MODE` at construction time
...
export function createShadowHarness(): ShadowHarness {
  const enabled = parseShadowModeEnv(process.env['CODRAGRAPH_SHADOW_MODE']);
...
/**
 * Env-var parser for `CODRAGRAPH_SHADOW_MODE`. Accepts the same truthy
