...
// ─── .codragraphignore negation can override hardcoded list (#771) ────
//
// Per @magyargergo's review: `.codragraphignore` should honour
// `.gitignore`-style negation against the hardcoded DEFAULT_IGNORE_LIST.
// A `!__tests__/` line in `.codragraphignore` must re-enable indexing of
...
//
// Locks in:
//   1. Default (no .codragraphignore) — hardcoded list still blocks
...
describe('.codragraphignore negation overrides hardcoded DEFAULT_IGNORE_LIST (#771)', () => {
  let tmpDir: string;
  ...
  it('default (no .codragraphignore): __tests__ still blocked by hardcoded list', async () => {
  ...
  it('`!__mocks__/` negation unlocks __mocks__ but NOT __tests__', async () => {
    await fs.writeFile(path.join(tmpDir, '.codragraphignore'), '!__mocks__/\n');
    const filter = await createIgnoreFilter(tmpDir);
  ...
  it('negation generalises — `!node_modules/` unlocks a different hardcoded entry', async () => {
    ...
    await fs.writeFile(path.join(tmpDir, '.codragraphignore'), '!node_modules/\n');
    const filter = await createIgnoreFilter(tmpDir);
  ...
  it('standard `.gitignore` rules (no negation) still layer on top of hardcoded', async () => {
    // Pre-#771 behaviour: if .codragraphignore says `my-dir/`, that dir
    // is ignored in addition to the hardcoded list. Non-negation
    // rules are unaffected by this PR.
    await fs.writeFile(path.join(tmpDir, '.codragraphignore'), 'my-dir/\n');
    const filter = await createIgnoreFilter(tmpDir);
  ...
  it('shouldIgnorePath (raw hardcoded check) is unchanged — wiki / external callers unaffected', async () => {
    ...
    // doesn't have access to per-repo `.codragraphignore` config. Its
...
describe('loadIgnoreRules', () => {
  ...
  it('parses .codragraphignore file', async () => {
    await fs.writeFile(path.join(tmpDir, '.codragraphignore'), 'vendor/\n*.test.ts\n');
    const ig = await loadIgnoreRules(tmpDir);
    expect(ig).not.toBeNull();
    ...
    await fs.unlink(path.join(tmpDir, '.codragraphignore'));
  });
...
describe('createIgnoreFilter', () => {
  ...
  it('childrenIgnored respects negation patterns without trailing slash (!dir vs !dir/)', async () => {
    ...
    await fs.writeFile(path.join(tmpDir, '.codragraphignore'), '*\n!iOS\n!iOS/**\n');
    const filter = await createIgnoreFilter(tmpDir);
...
describe('loadIgnoreRules — CODRAGRAPH_NO_GITIGNORE env var', () => {
  let tmpDir: string;
  ...
  it('skips .gitignore when CODRAGRAPH_NO_GITIGNORE is set', async () => {
    await fs.writeFile(path.join(tmpDir, '.gitignore'), 'data/\n');

    const original = process.env.CODRAGRAPH_NO_GITIGNORE;
    process.env.CODRAGRAPH_NO_GITIGNORE = '1';
    try {
      const ig = await loadIgnoreRules(tmpDir);
      ...
      if (original === undefined) {
        delete process.env.CODRAGRAPH_NO_GITIGNORE;
      } else {
        process.env.CODRAGRAPH_NO_GITIGNORE = original;
      }
      await fs.unlink(path.join(tmpDir, '.gitignore'));
    }
  });

  it('still reads .codragraphignore when CODRAGRAPH_NO_GITIGNORE is set', async () => {
    await fs.writeFile(path.join(tmpDir, '.codragraphignore'), 'vendor/\n');
