/**
 * Phase 4 Integration Test: lbug → graphstore round-trip.
 *
 * Closes the advisor flag from the Phase 4 MVP review: confirms that
 * `createLbugRowSource` actually unwraps `MATCH (n:T) RETURN n` results
 ...
 *
 * Uses the shared `withTestLbugDB` global setup — same pattern every
 * other lbug integration test in this repo follows.
 */
