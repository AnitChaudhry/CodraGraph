...
describe('filesystem-walker', () => {
  ...
  describe('.codragraphignore support', () => {
    ...
    beforeAll(async () => {
      ...
      // Only .codragraphignore, no .gitignore
      await fs.writeFile(path.join(nexusignoreDir, '.codragraphignore'), 'local/\n');
    });
    ...
    it('excludes directories listed in .codragraphignore', async () => {
  ...
  describe('combined .gitignore + .codragraphignore', () => {
    ...
    beforeAll(async () => {
      ...
      await fs.writeFile(path.join(combinedDir, '.codragraphignore'), 'local/\n');
    });
  ...
  describe('CODRAGRAPH_NO_GITIGNORE env var', () => {
    let envDir: string;
    ...
    it('includes gitignored directory when CODRAGRAPH_NO_GITIGNORE is set', async () => {
  ...
  describe('large file skip threshold (#991)', () => {
    ...
    const ORIGINAL_ENV = process.env.CODRAGRAPH_MAX_FILE_SIZE;
    ...
    beforeEach(() => {
      delete process.env.CODRAGRAPH_MAX_FILE_SIZE;
    ...
    afterEach(() => {
      if (ORIGINAL_ENV === undefined) {
        ...
        process.env.CODRAGRAPH_MAX_FILE_SIZE = ORIGINAL_ENV;
      }
      warnSpy.mockRestore();
    });
    ...
    it('includes the 600KB file when CODRAGRAPH_MAX_FILE_SIZE=1024', async () => {
      process.env.CODRAGRAPH_MAX_FILE_SIZE = '1024';
      const files = await walkRepositoryPaths(sizeDir);
      const paths = files.map((f) => f.path.replace(/\\/g, '/'));
      expect(paths).toContain(BIG_FILE);
    });

    it('falls back to default and warns once on invalid CODRAGRAPH_MAX_FILE_SIZE', async () => {
      process.env.CODRAGRAPH_MAX_FILE_SIZE = 'abc';
      const files = await walkRepositoryPaths(sizeDir);
      const paths = files.map((f) => f.path.replace(/\\/g, '/'));
      expect(paths).not.toContain(BIG_FILE);
    ...
    it('omits the "generated/vendored" suffix when threshold is overridden', async () => {
      process.env.CODRAGRAPH_MAX_FILE_SIZE = '1';
